...
fn a4(b: &[u8], o: usize) -> String {
    (0..4).map(|i| { let c=b[o+i]; if (32..127).contains(&c){c as char}else{'.'} }).collect()
}
...
fn main() {
    ...
    println!("{name} len={} riff_size@0x04={} (len-8={})",
        b.len(), u32_le(&b,0x04), b.len()-8);
    ...
    while off + 8 <= b.len() {
        ...
        println!("  {cc:6} off=0x{off:x} size={sz} body=0x{:x} next=0x{:x}", off+8, off+8+sz);
        if !cc.chars().all(|c| c.is_ascii_graphic()||c==' ') { println!("  (stop: garbage)"); break; }
        off += 8 + sz;
    }
    ...
    for (i,e) in entries.iter().enumerate() {
        ...
        println!("  [{i}] fsb5_offset=0x{:x} size={}  bytes_before=[{}] hex_before={:02x?}",
            e.fsb5_offset, e.fsb5_size, a4(&b, pre_off), &b[pre_off..e.fsb5_offset]);
    }
}
