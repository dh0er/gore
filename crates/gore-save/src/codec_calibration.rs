...
pub fn decode_self_test() -> bool {
    ...
    let Ok(out) = gore_oodle::decompress(&comp, SAMPLE_SIZE) else {
        return false;
    };
...
pub fn compress_roundtrip_self_test() -> bool {
    ...
    let Ok(comp) = gore_oodle::compress(&input, gore_oodle::Level::Default) else {
        return false;
    };
    let Ok(back) = gore_oodle::decompress(&comp, input.len()) else {
        return false;
    };
    back == input
}
