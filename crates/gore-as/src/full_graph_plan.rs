...
/// current emitter's compiler-ready representation of their sealed base module.
///
/// The complete-tree CLI is normally fed by `gore as emit-all`. Without this comparison every
/// one of those untouched files is classified as an Edit merely because it is present in the
/// authoritative tree. That defeats selective FullGraph publication: a no-op roundtrip would
/// replace every pristine module with regenerated bytes. Exact bytes are deliberately required;
/// formatting, comments, line endings, or any other authored difference remain an Edit.
pub fn plan_complete_source_tree_with_emitted_base_v1(
    base_cache: &[u8],
    binds_cache: &[u8],
    source_root: &Path,
) -> Result<PlannedFullGraphSourceTreeV1, FullGraphSourcePlanErrorV1> {
    ...
    let modules = crate::cache::model::parse_modules(base_cache)
        .map_err(|error| FullGraphSourcePlanErrorV1::EmittedBase(error.to_string()))?;
    let mut refs = crate::cache::refs::RefResolver::build(base_cache)
        .map_err(|error| FullGraphSourcePlanErrorV1::EmittedBase(error.to_string()))?;
    let native = crate::cache::binds::NativeApi::from_bytes(binds_cache).ok_or_else(|| {
        FullGraphSourcePlanErrorV1::EmittedBase("sealed Binds.Cache is invalid".to_owned())
    })?;
    let prepared = crate::cache::emit_all::PreparedEmit::new(&modules, &mut refs, Some(native))
        .map_err(|error| FullGraphSourcePlanErrorV1::EmittedBase(error.to_string()))?
        .with_class_defaults(true);
    let module_indices = modules
        .iter()
        .enumerate()
        .map(|(index, module)| (fold(&module.name), index))
        .collect::<BTreeMap<_, _>>();

    plan_inventory_with_unchanged(base, sources, |source| {
        let Some(&index) = module_indices.get(&fold(&source.module_name)) else {
            return Ok(false);
        };
        if prepared.module_relative_path(index) != Some(source.relative_path.as_str()) {
            return Ok(false);
        }
        prepared
            .emit_module(index)
            .map(|emitted| emitted.as_bytes() == source.bytes)
            .map_err(|error| FullGraphSourcePlanErrorV1::EmittedBase(error.to_string()))
    })
}
...
fn plan_inventory(
    ...
    plan_inventory_with_unchanged(base, sources, |_| Ok(false))
}
    ...
    for source in sources {
        ...
        let operation = match base_by_name.remove(&fold(&source.module_name)) {
            Some((base_name, base_path)) => {
                ...
                (!is_unchanged(&source)?).then_some(FullGraphCompileOperationV1::Edit)
            }
            None => Some(FullGraphCompileOperationV1::Add),
        };
...
pub enum FullGraphSourcePlanErrorV1 {
    ...
    BaseCache(String),
    #[error("base cache cannot supply an exact current emitted-source baseline: {0}")]
    EmittedBase(String),
...
mod tests {
    ...
    #[test]
    fn exact_emitted_source_is_base_while_real_differences_remain_changes() {
        let planned = plan_inventory_with_unchanged(
            vec![
                ("Same".into(), "Same.as".into()),
                ("Changed".into(), "Changed.as".into()),
                ("Deleted".into(), "Deleted.as".into()),
            ],
            vec![source("Same.as", b"same"), source("Changed.as", b"edit")],
            |source| Ok(source.module_name == "Same" && source.bytes == b"same"),
        )
        .unwrap();
        ...
        assert!(
            !planned
                .changes
                .iter()
                .any(|change| change.module_name == "Same")
        );
        assert!(planned.changes.iter().any(|change| {
            change.module_name == "Changed" && change.operation == FullGraphCompileOperationV1::Edit
        }));
        assert!(planned.changes.iter().any(|change| {
            change.module_name == "Deleted"
                && change.operation == FullGraphCompileOperationV1::Delete
        }));
    }

    #[test]
    #[ignore = "requires GORE_AS_CACHE, GORE_AS_BINDS, and GORE_AS_FULL_SOURCE_TREE fixtures"]
    fn real_emitted_tree_plans_only_the_authored_cross_module_delta() {
        let base = std::fs::read(std::env::var_os("GORE_AS_CACHE").expect("set GORE_AS_CACHE"))
            .unwrap();
        let binds = std::fs::read(std::env::var_os("GORE_AS_BINDS").expect("set GORE_AS_BINDS"))
            .unwrap();
        let source_root =
            PathBuf::from(std::env::var_os("GORE_AS_FULL_SOURCE_TREE").expect("set source tree"));
        let planned =
            plan_complete_source_tree_with_emitted_base_v1(&base, &binds, &source_root).unwrap();
        assert_eq!(planned.final_manifest().len(), 7_318);
        assert_eq!(
            planned
                .changes()
                .iter()
                .map(|change| (change.operation, change.module_name.as_str()))
                .collect::<Vec<_>>(),
            [
                (
                    FullGraphCompileOperationV1::Add,
                    "Gore.FullGraph.DiegoProbe"
                ),
                (
