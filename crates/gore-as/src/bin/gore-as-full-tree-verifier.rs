...
//! package recorder for an already-qualified final sidecar. It runs exactly one strict standalone
//! FullGraph compile through the production runner APIs and compares it with an already-produced
//! embedded cache. It never acquires an install-mutation guard and cannot start the game backend.
...
use gore_as::compile::{
    compile_full_graph_standalone_v1_with_target, CompilerBackendNameV1,
    FullGraphCompileOperationV1, FullGraphCompileOptsV1, FullGraphCompileOutcomeV1,
    FullGraphPublicationDispositionV1, InstallRestoreDisposition,
    ProjectCompilerClosingAuditDisposition,
};
...
<frozen-source-root> <embedded-reference.Cache> <existing-work-root> \
<new-standalone-output.Cache> <new-verification-receipt.json>";

fn main() {
    if let Err(error) = run() {
        eprintln!("full-tree standalone verification failed: {error:#}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<()> {
    ...
    // Keep both comparison authority and executable identity pinned for the entire operation.
    let embedded = PinnedBytes::open(
        &invocation.embedded_cache,
        MAX_CACHE_BYTES,
        "embedded reference cache",
    )?;
    let embedded_semantics = observe_whole_cache_semantics_v1(&embedded.bytes, None)
        .context("observing the complete embedded reference cache before compilation")?;
    ...
    let target = ValidatedCompilerTargetInputsV1::load(
    ...
    let opts = FullGraphCompileOptsV1 {
        ...
        work_dir: invocation.work_root.clone(),
    ...
    // This is the only compiler invocation in the helper. The selected entrypoint is incapable of
    // choosing or falling back to the embedded game compiler.
    let report =
        compile_full_graph_standalone_v1_with_target(&opts, &mut runner, closing_audit, target);
    let result = report.finish_while_target_pinned(|report| {
        finish_verification(
            report,
            &embedded,
            &embedded_semantics,
    ...
    write_receipt_no_clobber(&invocation.receipt_output, &result)?;
    println!(
...
impl Invocation {
    fn parse_from(args: impl IntoIterator<Item = OsString>) -> Result<Self> {
        ...
        let invocation = Self {
            ...
            embedded_cache: args[7].clone(),
            work_root: args[8].clone(),
            standalone_output: args[9].clone(),
            receipt_output: args[10].clone(),
        };
    ...
    fn labelled_paths(&self) -> [(&'static str, &Path); 11] {
        [
            ...
            ("embedded reference cache", &self.embedded_cache),
            ("standalone work root", &self.work_root),
    ...
    fn preflight(&self) -> Result<()> {
        for (label, path) in [
            ...
            ("standalone work root", self.work_root.as_path()),
        ] {
        ...
        if platform_path_eq(&self.standalone_output, &self.receipt_output) {
            bail!("standalone cache and verification receipt outputs must be distinct");
        }
        ...
        ensure_real_directory(output_parent, "standalone output parent")?;
        ...
        let embedded = self
            .embedded_cache
            .canonicalize()
            .context("resolving copied embedded reference cache")?;
        for (label, input) in [
            ("frozen source root", &source),
            ("qualified profile root", &profile),
            ("standalone work root", &work),
            ("standalone output parent", &output_parent),
            ("verification receipt output parent", &receipt_parent),
            ("standalone sidecar", &sidecar),
            ("copied embedded reference cache", &embedded),
        ] {
        ...
        if paths_overlap(&work, &source)
            ...
            || paths_overlap(&work, &output_parent)
            || paths_overlap(&work, &receipt_parent)
        {
            bail!("standalone work root must be disjoint from source, profile, and output roots");
        }
        if paths_overlap(&output_parent, &source) || paths_overlap(&output_parent, &profile) {
            bail!("standalone output parent must be disjoint from source and profile roots");
        }
...
fn finish_verification(
    ...
    embedded: &PinnedBytes,
    embedded_semantics: &WholeCacheSemanticObservationV1,
    ...
    let artifact = match report.outcome {
        ...
        FullGraphCompileOutcomeV1::Compiled(artifact) => {
            ...
            bail!(
                "standalone runner returned a cache without the exact strict-standalone/audit/\
                 publication contract; retained output was neutralized{}",
                cleanup
                    .map(|error| format!(" unsuccessfully: {error}"))
                    .unwrap_or_default()
            );
        }
    ...
    let whole_semantics_equal = embedded_semantics == &standalone_semantics;
    if let Err(gate) = require_differential_acceptance(bytediff_passed, whole_semantics_equal) {
        bail!(
            "differential verification rejected the standalone cache at {} (retained for local \
             diagnosis): {gate}; {}; {}",
            artifact.path().display(),
            bytediff_failure_summary(&byte_report),
            semantic_failure_summary(embedded_semantics, &standalone_semantics)
        );
    }
...
mod tests {
    ...
    fn valid_args(root: &Path) -> Vec<OsString> {
        ...
        vec![
            ...
            root.join("embedded.Cache").into_os_string(),
    ...
    fn arguments_are_exact_and_positional() {
        ...
        assert_eq!(parsed.standalone_output, PathBuf::from(&args[9]));
        assert_eq!(parsed.receipt_output, PathBuf::from(&args[10]));

        assert!(Invocation::parse_from(args[..10].iter().cloned())
            .unwrap_err()
            .to_string()
            .contains(USAGE));
        let mut extra = args;
    ...
    fn work_and_output_roots_are_disjoint_from_frozen_inputs() {
        for (target, input, expected) in [
            (8usize, 6usize, "work root must be disjoint"),
            (8, 1, "work root must be disjoint"),
            (9, 6, "output parent must be disjoint"),
            (9, 1, "output parent must be disjoint"),
            (10, 6, "receipt output parent must be disjoint"),
            (10, 1, "receipt output parent must be disjoint"),
        ] {
            let temp = tempfile::tempdir().unwrap();
            let root = temp.path().canonicalize().unwrap();
            let mut args = valid_args(&root);
            let input_root = PathBuf::from(&args[input]);
            args[target] = if target == 8 {
