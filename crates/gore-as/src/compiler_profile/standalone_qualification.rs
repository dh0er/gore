//! Productive offline adapter for the sealed 25-case standalone qualification corpus.
//!
...
fn bind_profile_corpus(
    ...
    if seal.byte_len != json.len() as u64 || seal.sha256 != digest {
        return Err(CompilerBackendFailureV1::unavailable(
            "unqualified profile does not seal the executable canonical 25-case corpus",
        ));
    }
    Ok(())
}
