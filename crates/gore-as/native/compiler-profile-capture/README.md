...
`CREATE_NEW`, and paths below the executable directory are rejected. Raw pointers are never
serialized: `intern_primary_image_pointer` accepts only addresses in the pinned primary image
and records an RVA-backed token.
