...
asCScriptNode *asCScriptNode::CreateCopy(FMemStackBase& MemStack, asCScriptEngine *engine)
{
	void *ptr = MemStack.Alloc(sizeof(asCScriptNode), alignof(asCScriptNode));
	...
	while( child )
	{
		node->AddChildLast(child->CreateCopy(MemStack, engine));
		child = child->next;
	}
