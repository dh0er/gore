...
private:
	...
	TMultiMap<TKey, OBJ> InnerMap;

	inline bool Matches(OBJ value, const char* name) const
	{
		return value->name == name;
	}

	inline bool Matches(OBJ value, const char* name, size_t len) const
	{
		return value->name.Equals(name, len);
	}

	inline bool Matches_CaseInsensitive(OBJ value, const char* name) const
	{
		const char* A = value->name.AddressOf();
		const char* B = name;
		while (*A != '\0')
		{
			if (asCString::RemoveCase(*A) != asCString::RemoveCase(*B))
				return false;
			++A;
			++B;
		}
	...
	inline bool Matches_CaseInsensitive(OBJ value, const char* name, size_t len) const
	{
		const char* A = value->name.AddressOf();
		const char* B = name;
...
public:
	...
	inline TKey GetKey(OBJ value) const
	{
		return GetKey(value->name.AddressOf(), value->name.GetLength());
	}

	inline TKey GetKey(const char* name) const
	{
		uint32 hash = 0x811c9dc5;
		for (const char* ptr = name; *ptr != '\0'; ++ptr)
		{
			hash ^= (uint32)(asCString::RemoveCase(*ptr));
			hash *= 16777619;
		}
		return hash;
	}

	inline TKey GetKey(const char* name, size_t len) const
	{
		uint32 hash = 0x811c9dc5;
		size_t i = 0;
		for (const char* ptr = name; i < len; ++ptr, ++i)
		{
			hash ^= (uint32)(asCString::RemoveCase(*ptr));
			hash *= 16777619;
		}
	...
	inline void Add(OBJ value)
	{
		TKey Key = GetKey(value);
		InnerMap.AddUnique(Key, value);
	}

	inline void Reserve(int Size)
	{
		InnerMap.Reserve(Size);
	}

	inline void Remove(OBJ value)
	{
		TKey Key = GetKey(value);
	...
	{
		return FindFirst(name) != nullptr;
	}

	inline OBJ FindFirst(const char* name) const
	{
		return FindFirst(GetKey(name), name);
	}

	inline OBJ FindFirst(const char* name, size_t len) const
	{
		return FindFirst(GetKey(name, len), name, len);
	}

	inline OBJ FindFirst(TKey Key, const char* name) const
	{
		return FindFirst(Key, name, strlen(name));
	}

	inline OBJ FindFirst_CaseInsensitive(const char* name) const
	{
		return FindFirst_CaseInsensitive(GetKey(name), name);
	}

	inline OBJ FindFirst_CaseInsensitive(TKey Key, const char* name) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
			if (Matches_CaseInsensitive(Iterator.Value(), name, length))
				return Iterator.Value();
		}
		return nullptr;
	}

	inline OBJ FindFirst(TKey Key, const char* name, size_t len) const
	{
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
			if (Matches(Iterator.Value(), name, len))
				return Iterator.Value();
		}
		return nullptr;
	}
	...
	inline void FindAll(const char* name, Functor callback) const
	{
		FindAll(GetKey(name), name, callback);
	}

	template<typename Functor>
	inline void FindAll(TKey Key, const char* name, Functor callback) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
			if (Matches(Iterator.Value(), name, length))
				callback(Iterator.Value());
		}
	}

	template<typename Functor>
	inline bool FindAllUntil(const char* name, Functor callback) const
	{
		return FindAllUntil(GetKey(name), name, callback);
	}

	template<typename Functor>
	inline bool FindAllUntil_CaseInsensitive(const char* name, Functor callback) const
	{
		return FindAllUntil_CaseInsensitive(GetKey(name), name, callback);
	}

	template<typename Functor>
	inline bool FindAllUntil(TKey Key, const char* name, Functor callback) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
	...
	template<typename Functor>
	inline bool FindAllUntil_CaseInsensitive(TKey Key, const char* name, Functor callback) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
	...
	template<typename Functor>
	inline void IterateAll(Functor callback) const
	{
		for (auto& Elem : InnerMap)
		{
			callback(Elem.Value);
		}
	}
	...
	template<typename Functor>
	inline bool IterateAllUntil(Functor callback) const
	{
		for (auto& Elem : InnerMap)
		{
...
private:
	...
	inline bool Matches(OBJ value, const char* name, const asSNameSpace* ns) const
	{
		return value->nameSpace == ns && value->name == name;
	}

	inline bool Matches(OBJ value, const char* name, size_t len, const asSNameSpace* ns) const
	{
		return value->nameSpace == ns && value->name.Equals(name, len);
	}

	inline bool Matches_CaseInsensitive(OBJ value, const char* name, const asSNameSpace* ns) const
	{
	...
	inline bool Matches_CaseInsensitive(OBJ value, const char* name, size_t len, const asSNameSpace* ns) const
	{
		if (value->nameSpace != ns)
			return false;
		const char* A = value->name.AddressOf();
	...
	inline TKey Hash(const char* name) const
	{
		uint32 hash = 0x811c9dc5;
		for (const char* ptr = name; *ptr != '\0'; ++ptr)
		{
	...
	inline TKey Hash(const char* name, size_t len) const
	{
		uint32 hash = 0x811c9dc5;
		size_t i = 0;
		for (const char* ptr = name; i < len; ++ptr, ++i)
...
public:
	...
	inline void SwapWith(asCSymbolMap& Other)
	{
		TMultiMap<TKey, OBJ> TempMap = MoveTemp(InnerMap);
		InnerMap = MoveTemp(Other.InnerMap);
		Other.InnerMap = MoveTemp(TempMap);
	}

	inline TKey GetKey(OBJ value) const
	{
		return HashCombineFast(Hash(value->name.AddressOf(), value->name.GetLength()), GetTypeHash(value->nameSpace));
	}

	inline TKey GetKey(const char* name, const asSNameSpace* ns) const
	{
		return HashCombineFast(Hash(name), GetTypeHash(ns));
	}

	inline TKey GetKey(const char* name, size_t len, const asSNameSpace* ns) const
	{
		return HashCombineFast(Hash(name, len), GetTypeHash(ns));
	}
	...
	inline bool Contains(OBJ value) const
	{
		TKey Key = GetKey(value);
		return InnerMap.FindPair(Key, value) != nullptr;
	}

	inline bool Contains(const char* name, const asSNameSpace* ns) const
	{
		return FindFirst(name, ns) != nullptr;
	}

	inline OBJ FindFirst(const char* name, const asSNameSpace* ns) const
	{
		return FindFirst(GetKey(name, ns), name, ns);
	}

	inline OBJ FindFirst(const char* name, size_t len, const asSNameSpace* ns) const
	{
		return FindFirst(GetKey(name, len, ns), name, ns);
	}

	inline OBJ FindFirst(TKey Key, const char* name, const asSNameSpace* ns) const
	{
		return FindFirst(Key, name, strlen(name), ns);
	}

	inline OBJ FindFirst_CaseInsensitive(TKey Key, const char* name, const asSNameSpace* ns) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
			if (Matches_CaseInsensitive(Iterator.Value(), name, length, ns))
				return Iterator.Value();
		}
		return nullptr;
	}

	inline OBJ FindFirst(TKey Key, const char* name, size_t len, const asSNameSpace* ns) const
	{
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
			if (Matches(Iterator.Value(), name, len, ns))
				return Iterator.Value();
		}
		return nullptr;
	}
	...
	template<typename Functor>
	inline void FindAll(const char* name, const asSNameSpace* ns, Functor callback) const
	{
		FindAll(GetKey(name, ns), name, ns, callback);
	}

	template<typename Functor>
	inline void FindAll(TKey Key, const char* name, const asSNameSpace* ns, Functor callback) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
			if (Matches(Iterator.Value(), name, length, ns))
				callback(Iterator.Value());
		}
	}

	template<typename Functor>
	inline bool FindAllUntil(const char* name, const asSNameSpace* ns, Functor callback) const
	{
		return FindAllUntil(GetKey(name, ns), name, ns, callback);
	}

	template<typename Functor>
	inline bool FindAllUntil(TKey Key, const char* name, const asSNameSpace* ns, Functor callback) const
	{
		int32 length = strlen(name);
		for (auto Iterator = InnerMap.CreateConstKeyIterator(Key); Iterator; ++Iterator)
		{
	...
	template<typename Functor>
	inline void IterateNamespace(const asSNameSpace* ns, Functor callback) const
	{
		for (auto& Elem : InnerMap)
		{
			if (Elem.Value->nameSpace != ns)
				continue;
			callback(Elem.Value);
		}
	}
