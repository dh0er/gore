...
int main() {
    ...
    options.native_super_types.push_back({
        ...
        standalone::native_super_kind::other_uobject, false});
    ...
    if (!mixed.succeeded() || built.size() != 5U ||
    ...
    const auto graph_export = precompiled::export_mixed_graph_checkpoint(
        cache, overlays, built, emitted_guid, 4, emitted);
    ...
    bool exported_string = false;
    for (const auto& entry : decoded.global_references) {
        exported_string = exported_string ||
    ...
    const bool reload_shape_ok = reload.succeeded() && reloaded.size() == 5U &&
    ...
    if (rejected_engine->RegisterObjectType(
            "NativeBase", 0, asOBJ_REF | asOBJ_NOCOUNT) < 0) {
