...
///
/// These values are produced from a deterministic synthetic plaintext (see
/// calibration_compress_input), NOT from any user save or game data. The
/// compressed payload was generated by the live host against the real game
/// build and verified to round-trip. Oodle Kraken decode format is
/// version-stable, so the compressed bytes decode identically across builds,
/// giving calibration a known-answer decompress oracle that needs no user save.
