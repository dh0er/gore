//! Exact-basis, project-only Quest transcript authoring for schema revision 3.
//!
//! A transcript is ordered authoring metadata on one [`QuestDraft`]. It binds existing or newly
//! created project-local [`DialogLine`] entities without changing Quest generation input, source,
//! ScriptModule, transition behavior, assets, or any game/save state. Native/store publication is
//! deliberately outside this pure transaction.

use std::collections::BTreeSet;
use std::io::{self, Write};

use serde::{Deserialize, Serialize};

use crate::model_revision3::{
    EntityKind, EntityPayload, ProjectRevision3, ProjectRevision3JsonError, QuestDraft,
    QuestTranscriptBindingV1, TypedRef, MAX_REVISION3_QUEST_TRANSCRIPT_BINDINGS_V1,
};
use crate::strict_json::reject_duplicate_object_keys;
use crate::{
    apply_revision3_dialog_line_insert_transaction_v1, regenerate_revision3_quest_module, EntityId,
    GameGenerationAnchor, ProjectId, QuestCollisionCatalogInput,
    Revision3DialogLineInsertConflictV1, Revision3DialogLineInsertErrorV1,
    Revision3DialogLineInsertEvaluationV1, Revision3DialogLineInsertRequestJsonErrorV1,
    Revision3DialogLineInsertRequestV1, Revision3DialogLocalizationActionV1, WorkingHead,
};

pub const MAX_REVISION3_QUEST_TRANSCRIPT_REQUEST_JSON_BYTES_V1: usize = 512 * 1024;

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(tag = "mode", rename_all = "snake_case", deny_unknown_fields)]
pub enum Revision3QuestTranscriptIntentV1 {
    Replace {
        bindings: Vec<QuestTranscriptBindingV1>,
    },
    CreateAndInsert {
        index: u16,
        objective_slot: u16,
        line: Revision3DialogLineInsertRequestV1,
    },
}

impl Revision3QuestTranscriptIntentV1 {
    pub const fn mode(&self) -> Revision3QuestTranscriptModeV1 {
        match self {
            Self::Replace { .. } => Revision3QuestTranscriptModeV1::Replace,
            Self::CreateAndInsert { .. } => Revision3QuestTranscriptModeV1::CreateAndInsert,
        }
    }
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Revision3QuestTranscriptEditRequestV1 {
    pub expected_head: WorkingHead,
    pub expected_project_id: ProjectId,
    pub expected_revision: u64,
    pub expected_target: GameGenerationAnchor,
    pub quest_id: EntityId,
    pub expected_quest_revision: u64,
    pub intent: Revision3QuestTranscriptIntentV1,
}

impl Revision3QuestTranscriptEditRequestV1 {
    pub fn from_json(json: &str) -> Result<Self, Revision3QuestTranscriptEditRequestJsonErrorV1> {
        if json.len() > MAX_REVISION3_QUEST_TRANSCRIPT_REQUEST_JSON_BYTES_V1 {
            return Err(
                Revision3QuestTranscriptEditRequestJsonErrorV1::InputTooLarge {
                    actual: json.len(),
                    limit: MAX_REVISION3_QUEST_TRANSCRIPT_REQUEST_JSON_BYTES_V1,
                },
            );
        }
        reject_duplicate_object_keys(json)
            .map_err(Revision3QuestTranscriptEditRequestJsonErrorV1::InvalidJson)?;
        let request: Self = serde_json::from_str(json)
            .map_err(Revision3QuestTranscriptEditRequestJsonErrorV1::InvalidJson)?;
        if request.to_canonical_json()?.as_bytes() != json.as_bytes() {
            return Err(Revision3QuestTranscriptEditRequestJsonErrorV1::NonCanonicalJson);
        }
        Ok(request)
    }

    pub fn to_canonical_json(
        &self,
    ) -> Result<String, Revision3QuestTranscriptEditRequestJsonErrorV1> {
        let mut writer =
            BoundedRequestWriter::new(MAX_REVISION3_QUEST_TRANSCRIPT_REQUEST_JSON_BYTES_V1);
        let serialized = serde_json::to_writer(&mut writer, self);
        if let Some(actual) = writer.first_exceeded_size {
            return Err(
                Revision3QuestTranscriptEditRequestJsonErrorV1::InputTooLarge {
                    actual,
                    limit: MAX_REVISION3_QUEST_TRANSCRIPT_REQUEST_JSON_BYTES_V1,
                },
            );
        }
        serialized.map_err(Revision3QuestTranscriptEditRequestJsonErrorV1::Serialize)?;
        String::from_utf8(writer.bytes)
            .map_err(|_| Revision3QuestTranscriptEditRequestJsonErrorV1::NonUtf8Serialization)
    }
}

#[derive(Debug, thiserror::Error)]
pub enum Revision3QuestTranscriptEditRequestJsonErrorV1 {
    #[error("revision-3 Quest transcript request exceeds the {limit}-byte limit: {actual} bytes")]
    InputTooLarge { actual: usize, limit: usize },
    #[error("invalid revision-3 Quest transcript request JSON: {0}")]
    InvalidJson(#[source] serde_json::Error),
    #[error("could not serialize revision-3 Quest transcript request JSON: {0}")]
    Serialize(#[source] serde_json::Error),
    #[error("revision-3 Quest transcript request JSON is not in its exact canonical spelling")]
    NonCanonicalJson,
    #[error("revision-3 Quest transcript request serializer emitted non-UTF-8 bytes")]
    NonUtf8Serialization,
}

#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum Revision3QuestTranscriptEditConflictV1 {
    #[error("request basis head does not match the exact supplied head")]
    CurrentHeadMismatch,
    #[error("expected project {expected}, but exact basis is {actual}")]
    ProjectIdentityMismatch {
        expected: ProjectId,
        actual: ProjectId,
    },
    #[error("expected project revision {expected}, but exact basis is {actual}")]
    ProjectRevisionConflict { expected: u64, actual: u64 },
    #[error("request target does not match the exact project target")]
    ProjectTargetMismatch,
    #[error("project revision cannot be incremented")]
    ProjectRevisionOverflow,
    #[error("Quest entity ID must not be zero")]
    ZeroQuestId,
    #[error("Quest {quest} is missing or has the wrong kind")]
    InvalidQuestEntity { quest: EntityId },
    #[error("expected Quest revision {expected}, but exact entity is {actual}")]
    QuestRevisionConflict { expected: u64, actual: u64 },
    #[error("Quest {quest} revision cannot be incremented")]
    QuestRevisionOverflow { quest: EntityId },
    #[error("Quest {quest} has an invalid source/module closure: {reason}")]
    InvalidQuestClosure { quest: EntityId, reason: String },
    #[error("Quest {quest} owned ScriptModule {module} differs from exact regeneration")]
    OwnedModuleDrift { quest: EntityId, module: EntityId },
    #[error("Quest transcript has {actual} bindings; maximum is {max}")]
    TooManyBindings { actual: usize, max: usize },
    #[error("Quest transcript binding {index} has an invalid line reference: {reason}")]
    InvalidLineReference { index: usize, reason: String },
    #[error("Quest transcript binding {index} duplicates DialogLine {line}")]
    DuplicateLine { index: usize, line: EntityId },
    #[error("Quest transcript binding {index} targets inactive objective slot {slot}")]
    InactiveObjectiveSlot { index: usize, slot: u16 },
    #[error("create-and-insert index {index} exceeds transcript length {len}")]
    InsertIndexOutOfBounds { index: usize, len: usize },
    #[error("embedded dialog-line request is not bound to the exact transcript basis")]
    DialogRequestBasisMismatch,
    #[error("embedded dialog-line request was rejected: {conflict}")]
    DialogLineRejected {
        conflict: Revision3DialogLineInsertConflictV1,
    },
    #[error("Quest transcript replacement makes no change")]
    NoChanges,
    #[error("candidate project exceeds the {limit}-byte limit: {actual} bytes")]
    CandidateTooLarge { actual: usize, limit: usize },
    #[error("candidate project is not persistable: {reason}")]
    CandidateNotPersistable { reason: String },
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Revision3QuestTranscriptEditRejectionV1 {
    pub conflict: Revision3QuestTranscriptEditConflictV1,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum Revision3QuestTranscriptModeV1 {
    Replace,
    CreateAndInsert,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Revision3QuestTranscriptCreatedLineV1 {
    pub line_id: EntityId,
    pub localization_id: EntityId,
    pub voice_slot_id: Option<EntityId>,
    pub localization_action: Revision3DialogLocalizationActionV1,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Revision3QuestTranscriptBuildStatusV1 {
    Blocked,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Revision3QuestTranscriptRuntimeStatusV1 {
    RuntimeUnqualified,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Revision3QuestTranscriptTopicAuthorityV1 {
    NotGranted,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Revision3QuestTranscriptPublicationStatusV1 {
    NotSupported,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Revision3QuestTranscriptEditOutcomeV1 {
    pub project: ProjectRevision3,
    pub canonical_project_json: String,
    pub basis_head: WorkingHead,
    pub quest_id: EntityId,
    pub script_module_id: EntityId,
    pub quest_revision: u64,
    pub script_module_revision: u64,
    pub mode: Revision3QuestTranscriptModeV1,
    pub transcript_count: u64,
    pub created: Option<Revision3QuestTranscriptCreatedLineV1>,
    pub build_status: Revision3QuestTranscriptBuildStatusV1,
    pub runtime_status: Revision3QuestTranscriptRuntimeStatusV1,
    pub topic_authority: Revision3QuestTranscriptTopicAuthorityV1,
    pub publication_status: Revision3QuestTranscriptPublicationStatusV1,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum Revision3QuestTranscriptEditEvaluationV1 {
    Applied(Box<Revision3QuestTranscriptEditOutcomeV1>),
    Rejected(Revision3QuestTranscriptEditRejectionV1),
}

#[derive(Debug, thiserror::Error)]
pub enum Revision3QuestTranscriptEditErrorV1 {
    #[error("invalid exact canonical revision-3 project: {0}")]
    InvalidProject(#[source] ProjectRevision3JsonError),
    #[error("invalid exact canonical revision-3 Quest transcript request: {0}")]
    InvalidRequest(#[source] Revision3QuestTranscriptEditRequestJsonErrorV1),
    #[error("embedded dialog-line request could not be serialized: {0}")]
    InvalidEmbeddedDialogRequest(#[source] Revision3DialogLineInsertRequestJsonErrorV1),
    #[error("embedded dialog-line transaction failed: {0}")]
    DialogLineTransaction(#[source] Revision3DialogLineInsertErrorV1),
    #[error("could not reopen generated canonical revision-3 project: {0}")]
    ReopenCandidate(#[source] ProjectRevision3JsonError),
    #[error("canonical revision-3 Quest transcript candidate reopen changed the project")]
    CanonicalReopenMismatch,
    #[error("Quest transcript candidate changed state outside its exact transaction boundary")]
    CandidatePreservationMismatch,
}

pub fn apply_revision3_quest_transcript_edit_transaction_v1(
    exact_basis_head: &WorkingHead,
    canonical_project_json: &str,
    canonical_request_json: &str,
) -> Result<Revision3QuestTranscriptEditEvaluationV1, Revision3QuestTranscriptEditErrorV1> {
    let project = ProjectRevision3::from_json(canonical_project_json)
        .map_err(Revision3QuestTranscriptEditErrorV1::InvalidProject)?;
    let request = Revision3QuestTranscriptEditRequestV1::from_json(canonical_request_json)
        .map_err(Revision3QuestTranscriptEditErrorV1::InvalidRequest)?;

    macro_rules! reject {
        ($conflict:expr) => {
            return Ok(Revision3QuestTranscriptEditEvaluationV1::Rejected(
                Revision3QuestTranscriptEditRejectionV1 {
                    conflict: $conflict,
                },
            ))
        };
    }

    if &request.expected_head != exact_basis_head {
        reject!(Revision3QuestTranscriptEditConflictV1::CurrentHeadMismatch);
    }
    if request.expected_project_id != project.project_id {
        reject!(
            Revision3QuestTranscriptEditConflictV1::ProjectIdentityMismatch {
                expected: request.expected_project_id,
                actual: project.project_id,
            }
        );
    }
    if request.expected_revision != project.revision {
        reject!(
            Revision3QuestTranscriptEditConflictV1::ProjectRevisionConflict {
                expected: request.expected_revision,
                actual: project.revision,
            }
        );
    }
    if request.expected_target != project.target {
        reject!(Revision3QuestTranscriptEditConflictV1::ProjectTargetMismatch);
    }
    let Some(next_project_revision) = project.revision.checked_add(1) else {
        reject!(Revision3QuestTranscriptEditConflictV1::ProjectRevisionOverflow);
    };
    if is_zero_entity_id(request.quest_id) {
        reject!(Revision3QuestTranscriptEditConflictV1::ZeroQuestId);
    }

    let Some(quest_entity) = project.entities.get(&request.quest_id) else {
        reject!(Revision3QuestTranscriptEditConflictV1::InvalidQuestEntity {
            quest: request.quest_id,
        });
    };
    let EntityPayload::QuestDraft(quest) = &quest_entity.payload else {
        reject!(Revision3QuestTranscriptEditConflictV1::InvalidQuestEntity {
            quest: request.quest_id,
        });
    };
    if request.expected_quest_revision != quest_entity.revision {
        reject!(
            Revision3QuestTranscriptEditConflictV1::QuestRevisionConflict {
                expected: request.expected_quest_revision,
                actual: quest_entity.revision,
            }
        );
    }
    let Some(next_quest_revision) = quest_entity.revision.checked_add(1) else {
        reject!(
            Revision3QuestTranscriptEditConflictV1::QuestRevisionOverflow {
                quest: request.quest_id,
            }
        );
    };
    let script_module_id = quest.script_module.id;
    if quest.script_module.project_id != project.project_id
        || quest.script_module.expected_kind != EntityKind::ScriptModule
        || is_zero_entity_id(script_module_id)
    {
        reject!(
            Revision3QuestTranscriptEditConflictV1::InvalidQuestClosure {
                quest: request.quest_id,
                reason: "ScriptModule reference is not exact-project, non-zero, and kind-bound"
                    .to_owned(),
            }
        );
    }
    let Some(module_entity) = project.entities.get(&script_module_id) else {
        reject!(
            Revision3QuestTranscriptEditConflictV1::InvalidQuestClosure {
                quest: request.quest_id,
                reason: "owned ScriptModule is missing".to_owned(),
            }
        );
    };
    let EntityPayload::ScriptModule(existing_module) = &module_entity.payload else {
        reject!(
            Revision3QuestTranscriptEditConflictV1::InvalidQuestClosure {
                quest: request.quest_id,
                reason: "owned entity is not a ScriptModule".to_owned(),
            }
        );
    };
    if existing_module.owner.project_id != project.project_id
        || existing_module.owner.id != request.quest_id
        || existing_module.owner.expected_kind != EntityKind::QuestDraft
    {
        reject!(
            Revision3QuestTranscriptEditConflictV1::InvalidQuestClosure {
                quest: request.quest_id,
                reason: "ScriptModule owner is not the exact Quest".to_owned(),
            }
        );
    }
    let regenerated = match regenerate_revision3_quest_module(quest, empty_collision_input(quest)) {
        Ok(module) => module,
        Err(error) => {
            reject!(
                Revision3QuestTranscriptEditConflictV1::InvalidQuestClosure {
                    quest: request.quest_id,
                    reason: error.to_string(),
                }
            );
        }
    };
    if &regenerated != existing_module {
        reject!(Revision3QuestTranscriptEditConflictV1::OwnedModuleDrift {
            quest: request.quest_id,
            module: script_module_id,
        });
    }
    let script_module_revision = module_entity.revision;
    let basis_transcript = quest.transcript.clone();
    let mode = request.intent.mode();

    let (mut candidate, transcript, created) = match &request.intent {
        Revision3QuestTranscriptIntentV1::Replace { bindings } => {
            if let Err(conflict) = validate_bindings(&project, quest, bindings) {
                reject!(conflict);
            }
            if bindings == &basis_transcript {
                reject!(Revision3QuestTranscriptEditConflictV1::NoChanges);
            }
            (project.clone(), bindings.clone(), None)
        }
        Revision3QuestTranscriptIntentV1::CreateAndInsert {
            index,
            objective_slot,
            line,
        } => {
            let index = usize::from(*index);
            if index > basis_transcript.len() {
                reject!(
                    Revision3QuestTranscriptEditConflictV1::InsertIndexOutOfBounds {
                        index,
                        len: basis_transcript.len(),
                    }
                );
            }
            if basis_transcript.len() >= MAX_REVISION3_QUEST_TRANSCRIPT_BINDINGS_V1 {
                reject!(Revision3QuestTranscriptEditConflictV1::TooManyBindings {
                    actual: basis_transcript.len().saturating_add(1),
                    max: MAX_REVISION3_QUEST_TRANSCRIPT_BINDINGS_V1,
                });
            }
            if line.expected_head != *exact_basis_head
                || line.expected_project_id != project.project_id
                || line.expected_revision != project.revision
                || line.expected_target != project.target
            {
                reject!(Revision3QuestTranscriptEditConflictV1::DialogRequestBasisMismatch);
            }
            let line_json = line
                .to_canonical_json()
                .map_err(Revision3QuestTranscriptEditErrorV1::InvalidEmbeddedDialogRequest)?;
            let dialog = match apply_revision3_dialog_line_insert_transaction_v1(
                exact_basis_head,
                canonical_project_json,
                &line_json,
            )
            .map_err(Revision3QuestTranscriptEditErrorV1::DialogLineTransaction)?
            {
                Revision3DialogLineInsertEvaluationV1::Applied(outcome) => *outcome,
                Revision3DialogLineInsertEvaluationV1::Rejected(rejection) => {
                    reject!(Revision3QuestTranscriptEditConflictV1::DialogLineRejected {
                        conflict: rejection.conflict,
                    });
                }
            };
            if dialog.project.revision != next_project_revision
                || dialog.basis_head != *exact_basis_head
                || dialog.line_id != line.line_id
                || dialog.project.entities.get(&request.quest_id)
                    != project.entities.get(&request.quest_id)
                || dialog.project.entities.get(&script_module_id)
                    != project.entities.get(&script_module_id)
            {
                return Err(Revision3QuestTranscriptEditErrorV1::CanonicalReopenMismatch);
            }
            let mut transcript = basis_transcript.clone();
            transcript.insert(
                index,
                QuestTranscriptBindingV1 {
                    line: TypedRef::new(project.project_id, line.line_id, EntityKind::DialogLine),
                    objective_slot: *objective_slot,
                },
            );
            if let Err(conflict) = validate_bindings(&dialog.project, quest, &transcript) {
                reject!(conflict);
            }
            let created = Revision3QuestTranscriptCreatedLineV1 {
                line_id: dialog.line_id,
                localization_id: dialog.localization_id,
                voice_slot_id: dialog.voice_slot_id,
                localization_action: dialog.localization_action,
            };
            (dialog.project, transcript, Some(created))
        }
    };

    let transcript_edit_basis = candidate.clone();

    let Some(candidate_quest_entity) = candidate.entities.get_mut(&request.quest_id) else {
        return Err(Revision3QuestTranscriptEditErrorV1::CanonicalReopenMismatch);
    };
    let EntityPayload::QuestDraft(candidate_quest) = &mut candidate_quest_entity.payload else {
        return Err(Revision3QuestTranscriptEditErrorV1::CanonicalReopenMismatch);
    };
    candidate_quest.transcript = transcript;
    candidate_quest_entity.revision = next_quest_revision;
    candidate.revision = next_project_revision;

    let canonical_project_json = match candidate.to_canonical_json() {
        Ok(json) => json,
        Err(ProjectRevision3JsonError::InputTooLarge { actual, limit }) => {
            reject!(Revision3QuestTranscriptEditConflictV1::CandidateTooLarge { actual, limit });
        }
        Err(error) => {
            reject!(
                Revision3QuestTranscriptEditConflictV1::CandidateNotPersistable {
                    reason: error.to_string(),
                }
            );
        }
    };
    let reopened = ProjectRevision3::from_json(&canonical_project_json)
        .map_err(Revision3QuestTranscriptEditErrorV1::ReopenCandidate)?;
    if reopened != candidate {
        return Err(Revision3QuestTranscriptEditErrorV1::CanonicalReopenMismatch);
    }
    if !preserves_exact_transcript_edit(
        &transcript_edit_basis,
        &reopened,
        request.quest_id,
        next_project_revision,
        next_quest_revision,
    ) {
        return Err(Revision3QuestTranscriptEditErrorV1::CandidatePreservationMismatch);
    }
    let transcript_count = reopened
        .entities
        .get(&request.quest_id)
        .and_then(|entity| match &entity.payload {
            EntityPayload::QuestDraft(quest) => Some(quest.transcript.len() as u64),
            _ => None,
        })
        .ok_or(Revision3QuestTranscriptEditErrorV1::CanonicalReopenMismatch)?;

    Ok(Revision3QuestTranscriptEditEvaluationV1::Applied(Box::new(
        Revision3QuestTranscriptEditOutcomeV1 {
            project: reopened,
            canonical_project_json,
            basis_head: exact_basis_head.clone(),
            quest_id: request.quest_id,
            script_module_id,
            quest_revision: next_quest_revision,
            script_module_revision,
            mode,
            transcript_count,
            created,
            build_status: Revision3QuestTranscriptBuildStatusV1::Blocked,
            runtime_status: Revision3QuestTranscriptRuntimeStatusV1::RuntimeUnqualified,
            topic_authority: Revision3QuestTranscriptTopicAuthorityV1::NotGranted,
            publication_status: Revision3QuestTranscriptPublicationStatusV1::NotSupported,
        },
    )))
}

fn validate_bindings(
    project: &ProjectRevision3,
    quest: &QuestDraft,
    bindings: &[QuestTranscriptBindingV1],
) -> Result<(), Revision3QuestTranscriptEditConflictV1> {
    if bindings.len() > MAX_REVISION3_QUEST_TRANSCRIPT_BINDINGS_V1 {
        return Err(Revision3QuestTranscriptEditConflictV1::TooManyBindings {
            actual: bindings.len(),
            max: MAX_REVISION3_QUEST_TRANSCRIPT_BINDINGS_V1,
        });
    }
    let active_slots = quest
        .input
        .transition_plan
        .objective_slots
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let mut line_ids = BTreeSet::new();
    for (index, binding) in bindings.iter().enumerate() {
        if binding.line.project_id != project.project_id {
            return Err(
                Revision3QuestTranscriptEditConflictV1::InvalidLineReference {
                    index,
                    reason: "line belongs to another project".to_owned(),
                },
            );
        }
        if binding.line.expected_kind != EntityKind::DialogLine {
            return Err(
                Revision3QuestTranscriptEditConflictV1::InvalidLineReference {
                    index,
                    reason: "typed reference does not expect DialogLine".to_owned(),
                },
            );
        }
        if !line_ids.insert(binding.line.id) {
            return Err(Revision3QuestTranscriptEditConflictV1::DuplicateLine {
                index,
                line: binding.line.id,
            });
        }
        let Some(line) = project.entities.get(&binding.line.id) else {
            return Err(
                Revision3QuestTranscriptEditConflictV1::InvalidLineReference {
                    index,
                    reason: "DialogLine is missing".to_owned(),
                },
            );
        };
        if !matches!(line.payload, EntityPayload::DialogLine(_)) {
            return Err(
                Revision3QuestTranscriptEditConflictV1::InvalidLineReference {
                    index,
                    reason: "target entity has the wrong kind".to_owned(),
                },
            );
        }
        if !active_slots.contains(&binding.objective_slot) {
            return Err(
                Revision3QuestTranscriptEditConflictV1::InactiveObjectiveSlot {
                    index,
                    slot: binding.objective_slot,
                },
            );
        }
    }
    Ok(())
}

fn empty_collision_input(quest: &QuestDraft) -> QuestCollisionCatalogInput {
    QuestCollisionCatalogInput {
        generation: quest.input.collision_catalog.generation.clone(),
        source_seal: quest.input.collision_catalog.source_seal.clone(),
        catalog_layer: quest.input.collision_catalog.catalog_layer.clone(),
        modules: BTreeSet::new(),
        relative_paths: BTreeSet::new(),
        symbols: BTreeSet::new(),
    }
}

fn preserves_exact_transcript_edit(
    basis: &ProjectRevision3,
    candidate: &ProjectRevision3,
    quest_id: EntityId,
    expected_project_revision: u64,
    expected_quest_revision: u64,
) -> bool {
    if candidate.project_id != basis.project_id
        || candidate.revision != expected_project_revision
        || candidate.meta != basis.meta
        || candidate.target != basis.target
        || candidate.authoring_locales != basis.authoring_locales
        || candidate.asset_store != basis.asset_store
        || candidate.entities.len() != basis.entities.len()
        || candidate.entities.keys().ne(basis.entities.keys())
    {
        return false;
    }

    for (id, entity) in &basis.entities {
        let Some(candidate_entity) = candidate.entities.get(id) else {
            return false;
        };
        if *id != quest_id {
            if candidate_entity != entity {
                return false;
            }
            continue;
        }
        if candidate_entity.id != entity.id
            || candidate_entity.display_name != entity.display_name
            || candidate_entity.origin != entity.origin
            || candidate_entity.revision != expected_quest_revision
        {
            return false;
        }
        let (EntityPayload::QuestDraft(basis_quest), EntityPayload::QuestDraft(candidate_quest)) =
            (&entity.payload, &candidate_entity.payload)
        else {
            return false;
        };
        let mut expected_quest = basis_quest.clone();
        expected_quest.transcript = candidate_quest.transcript.clone();
        if candidate_quest != &expected_quest {
            return false;
        }
    }
    true
}

fn is_zero_entity_id(id: EntityId) -> bool {
    id.as_bytes().iter().all(|byte| *byte == 0)
}

struct BoundedRequestWriter {
    bytes: Vec<u8>,
    limit: usize,
    first_exceeded_size: Option<usize>,
}

impl BoundedRequestWriter {
    fn new(limit: usize) -> Self {
        Self {
            bytes: Vec::with_capacity(limit.min(64 * 1024)),
            limit,
            first_exceeded_size: None,
        }
    }
}

impl Write for BoundedRequestWriter {
    fn write(&mut self, bytes: &[u8]) -> io::Result<usize> {
        let actual = self.bytes.len().saturating_add(bytes.len());
        if actual > self.limit {
            self.first_exceeded_size.get_or_insert(actual);
            return Err(io::Error::other("Quest transcript request limit exceeded"));
        }
        self.bytes.extend_from_slice(bytes);
        Ok(bytes.len())
    }

    fn flush(&mut self) -> io::Result<()> {
        Ok(())
    }
}
