...
| [Characters](docs/guide/npc-authoring.md) | Read, plus unproven authoring | See which characters the game ships, the class chain one is made of, and which world points spawn it; write the source that adds a new character to the world or stops a shipped one being placed | Reading is proven. Authoring is prepared and checked offline but has never been run in game: that a new character appears, keeps its routine or survives a save is unproven. Editing an existing character is not covered at all |
