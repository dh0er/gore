...
/// Points every platform's base-dir env var at [base] so `goreToolsDir`
...
void main() {
  test('fills missing files from the legacy gore-tools umbrella', () {
    ...
    File(p.join(legacy.path, 'gore-save', 'ui_settings.json'))
    ...
    File(p.join(newDir.path, 'gore-save', 'ui_settings.json'))
      ..createSync(recursive: true)
  ...
  test('is a no-op when the legacy umbrella is absent', () {
    ...
    expect(
      Directory(goreToolsDir(environment: env)).existsSync(),
      isFalse,
