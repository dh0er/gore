...
void main() {
  ...
  testWidgets('groups NPC rows into the player sidebar categories',
    ...
    expect(find.widgetWithText(TextField, 'Health base'), findsNothing);
  });
  ...
  testWidgets('NPC-only attributes fall into the Advanced group',
    ...
    await tester.pumpWidget(
      _wrap(
        _panel(
          load: () async => NpcAttributesResult(
            attributes: [_row('DamageMultiplier', 1, 1)],
          ),
        ),
      ),
    );
    ...
    expect(
      find.widgetWithText(TextField, 'DamageMultiplier base'),
      findsOneWidget,
    );
  });
  ...
  testWidgets('Thieving-only attributes produce NO Thieving group for NPCs',
    ...
    expect(
      find.widgetWithText(TextField, 'PickPocketing base'),
      findsNothing,
    );
  });
  ...
  testWidgets('Status row is the FIRST entry of the core (Main stats) group',
    ...
    final healthFieldY = tester
        .getTopLeft(find.widgetWithText(TextField, 'Health base'))
        .dy;
  ...
  testWidgets('Status row Revive registers pending without an immediate write',
      (tester) async {
    var revived = 0;
    await tester.pumpWidget(
      _wrap(
