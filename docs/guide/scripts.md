...
```
warning: AI.States.FightAI.CombatState.CombatMoves carries 36 functions the decompiler does not
reproduce as the same program. Splicing this module recompiles all of it, so those come out
changed as well.
```

Silence is the good case: **7,064 of the 7,317 modules recompile with no known semantic
...
The remaining 253 differ only in spelling — same program, different text. The loops that once
