...
| `value` | `inspect` | Show one class's recovered defaults from the Shipping script cache. | [items](items.md) |
| `mod` | `build` · `inspect` · `deploy` · `undeploy` | Build, validate, inspect, deploy, or undeploy a unified bundle. `values` compiles into a script mini-cache. | [bundles](bundles.md) |
...
Commands that need the install (`mod`, `mgr`, `texture`,
...
| `clone` | `<SOURCE>` · `--id <ID>` · `--guild <BASE>` · `--at <POINT>` · `--waypoint <SPOT>` · `--trader` · `--cache` · `--game` · `--out <DIR>` | Like `new`, but the template resolved values are written into the file instead of inherited silently — 51 lines for Diego. Use it when the point is to change the numbers; use `new` when the point is a character that follows its template. |
...
therefore has no loader beside the game. A workspace that still carries
`dialog_topics` is refused by `check` and `stage`.
...
`gore value inspect --class <CLASS> [--cache <CACHE>] [--game <GAME>] [--json]`

Shows the module, recovered class defaults, their types and current values.
Trader stock, instance inventories and save values are listed as outside this
command.
