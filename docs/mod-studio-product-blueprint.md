...
|---|---|---|---|
| Project and content library | The normal Flutter project still uses a legacy format-1 archive and separate provider/tab state. Its compatibility session owns the current path and extracted workspace, serializes Open/Save/New, distinguishes Save from Save As, preserves edits made during I/O as dirty, and uses strict bounded ZIP validation, canonical embedded paths, and crash-recoverable byte swaps. Separately, the visible Story create/open flow owns an exclusive schema-revision-2 managed working directory, serialized derive/save lane, exact-head CAS publication, repair journal, and full reopen. A dedicated revision-3 managed-session API reuses that safety core. A typed app-wide current-project coordinator foundation owns at most one authoritative current Legacy or managed revision-3 lease; a candidate may coexist only during full validation before adoption. It serializes open/adopt/save/verify/close, preserves the current lease on failed candidate open, and maps managed Save to exact-head full-reopen verification without a new checkpoint. It is not yet wired into Home, menus, `Ctrl+S`, or app-shell lifecycle and therefore does not yet provide one launched unified library, transaction history, autosave/recovery model, or all-domain session. | `gore-authoring` has closed revision-1/2/3 documents, explicit revision-2-to-3 migration, strict IDs/refs and canonical JSON, plus an immutable working-directory store with sealed heads, snapshot/entity shards, physical Ogg and Quest-artifact CAS blobs, and prepare-only revision-3 checkpoints. Dedicated revision-3 Store FFI commands, strict Studio wrappers/DTOs, and the managed R3 session can open the fixed head, reopen exact candidate-head bytes, prepare immutable checkpoints, publish by exact byte CAS, repair interrupted publication, and fully reopen the result. The coordinator retires leases once and retains terminal cleanup failures only as diagnostics. Neither layer infers diagnostics, readiness, runtime, deployment, or artifact authority. Migration orchestration, full history/recovery, app-shell adoption, and a unified content library remain integration work. | Runtime is not applicable. End state is safe create/open/recover, global search, collections, backlinks, lineage, source layers, and deterministic export from one managed project. |
...
| Quests | No semantic quest catalog/editor or friendly revision-3 managed-project wizard is integrated yet. The existing visible managed Story create/open flow remains schema revision 2; neither the safe R3 managed-session API nor the current-project coordinator foundation is launched by that UI. | The earlier revision-3 path stores compact Quest artifact references, can import/reopen the exact collision artifact and old basis snapshot, persist the first offline Quest Draft plus deterministic `ScriptModule`, and regenerate it for source inspection only after a fresh source-bound capability verifies every retained identity. Version 2 provides an opaque fresh collision capability and linear prepared artifact bound to the Base Game, trusted Story catalog, exact project identity/revision/target/head and bytes, non-Quest state, prior-Quest evidence, and every collision domain; structural reopen does not restore authority. A filesystem-free transaction consumes that capability and exact canonical transports, constructs the next Quest Draft and deterministic ScriptModule, increments the revision, and returns an externally opaque result after canonical reopen equality. The lower Store imports its exact artifact through bounded no-clobber CAS with full raw verification against the unchanged basis head. The consuming Story persistence helper separately revalidates the canonical candidate, artifact, exact basis, entity/asset delta, QuestRef, and regenerated module, then prepares and fully reopens a revision-3 checkpoint. It never publishes the fixed head; a late race can leave only verified immutable CAS orphans, never a published candidate head. FFI orchestration, managed-session publication, and Studio integration remain separate work. All outcomes stay build-blocked/runtime-unqualified, grant no artifact authority, and require a fresh capability for source inspection. A retained probe independently showed two added quest subclasses instantiated as `Available` on one generation; that does not qualify generated transitions or effects. | Availability/start/success/failure transitions, dialog selection, journal, rewards, knowledge, effects, save/reload, persistence, and uninstall behavior are **separate quest research gates**. They do not become qualified when NPC spawning works. |
...
| DataAssets and cooked content | A read-only DataAsset Lab opens a selected local `.uasset`/`.uexp` snapshot with its exact `.usmap`, separates walked/partial/unsupported exports, and lazily searches proven fixed-width leaves. It exposes no patch, project stage, save, pack, deploy, raw-offset, or Unreal-bridge control; native paths do not enter the evidence DTO, and a semantic browser/editor is still missing. | The native inspector uses guarded no-follow reads, allocation/work/hash/depth/count budgets, exact pair/USMAP seals, and offset-free selectors. Extract-v2/Patch-v2 wires, bounded readers, pair/USMAP/sidecar/generation validators, and opaque verified-file capabilities now form one shared `gore-asset` workflow used by the CLI, with complete normalized receipt bytes frozen by golden hashes. Receipt-bound copy-on-write fixed-width leaf edits remain the proven write subset; shared validation does not itself create managed staging. General package/export/name/reference/array/map/collection writing and the optional sealed Unreal handoff are not implemented. | Existing fixed-leaf runtime semantics are only as broad as their reviewed field proof. Structural creation remains **Research-gated** until complete round-trip and runtime qualification. A stock Unreal Editor is not assumed to open cooked G1R packages or emit compatible output. |
...
full reopen through the working store. A dedicated revision-3 session API now
reuses the same safety core and exposes only durable project/checkpoint identity.
A typed current-project coordinator foundation now serializes Legacy/R3
ownership transitions and backend-appropriate Save/verify operations while
keeping failed candidate opens from disturbing the current lease. It is still
not adopted by the app shell, Home, menus, or `Ctrl+S`, so there is no visible
unified R3 authoring flow.
...
publication, repair, and full reopen exist. A typed Legacy/R3 current-project
coordinator also exists, but app-shell adoption, migration orchestration into
the managed session, autosave/full history, and all-domain transactions remain
