...
|---|---|---|---|
| Project and content library | The compatibility project remains a legacy format-1 bridge and the separate Story flow remains schema revision 2. The primary Project menu creates or opens one authoritative managed-R3 project. Managed Studio Shell v1 now exposes the canonical Home, Content, Story, World, Localization & Voice, Validate & Test, Build & Release, and Settings & Expert destinations. DataAssets is a secondary Content view. Lazy-mounted pages preserve the primary destination, each section's secondary route, and page state across same-project revisions; a project-identity change resets to Home. Unsupported domains stay visible and honestly labelled. | Home reads the exact current index. Story exposes bounded NPC/Quest Draft actions; Localization & Voice exposes take/selection/target actions; Validate can verify the current head and inspect references; Build exposes only the sealed Voice bundle; and Settings is available. World authoring, runtime test, full build/deploy, and Expert tools remain unavailable. Creation still uses a secure project ID, canonical empty revision-3 document, absent-head CAS, full reopen, and single-owner adoption. General editors, migration/import/clone/Save As, history/recovery UI, unified cross-source search and scopes, collections, unified transactions, and blob-ownership tools remain. | Project creation and shell navigation write neither the game installation nor a save and grant no deployment or runtime authority. General managed build remains unavailable; only the separately sealed existing-member Voice bundle has a bounded offline build path. The next slices are the honest `This mod` / `Base game` / `Installed` Content scopes, then bounded Empty/NPC Draft/Quest Draft starter templates. |
...
build, Settings, and verified DataAsset actions now share that R3 owner. World,
runtime test, full managed build/deploy, and Expert tools remain visible but
unavailable. This is the complete primary shell, not yet a unified cross-source
...
Current implementation note: Managed Studio Shell v1 implements all eight
primary destinations above. DataAssets is a secondary Content view, and the
shell preserves selected primary and per-section secondary state across
same-project revisions. Pages without production capability remain visible with
honest availability status. The unified cross-source browser, command palette,
and complete workspace chrome remain missing.

The immediate next Content slice has three honest scopes: `This mod`, `Base
game`, and `Installed`. It does not show `Dependencies` until a managed
dependency model can support that claim. The following starter-project slice
offers **Empty**, **NPC Draft**, and **Quest Draft**. Empty reuses absent-head
atomic creation. A single Draft may be prepared before adoption against the
fresh empty head, but a failed seed can leave a valid recoverable empty project;
the UI must not describe that as one all-domain atomic transaction. Multi-domain
templates require a native compound prepare/publication transaction.
