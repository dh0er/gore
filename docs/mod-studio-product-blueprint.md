...
| DataAssets and cooked content | The read-only DataAsset Lab still opens a selected local `.uasset`/`.uexp` snapshot with its exact `.usmap`, separates walked/partial/unsupported exports, and lazily searches proven fixed-width leaves. Managed R3 Home now also has a searchable **Verified DataAsset edits** registry: it imports independently produced PatchReceipt JSON, shows friendly verified facts, and confirms registry-only removal. Exact initial loading, root/project/revision/head binding, drift, and `requiresReopen` gate every mutation. This is not a semantic value editor and exposes no direct value authoring, diff/undo, build, pack, deploy, gameplay, raw-offset, or Unreal-bridge control. | The native inspector and CLI share guarded no-follow reads, bounded Extract-v2/Patch-v2 validation, exact pair/USMAP/sidecar/generation proofs, and offset-free selectors. Native revision-3 authoring freshly reconstructs and union-probes the live target behind one opaque verified patch, binds the exact executable, rejects Store/game aliasing, and prepares a fully reopened immutable stage checkpoint. Closed bounded FFI prepare/list/remove routes expose that boundary without receipt/path/offset leaks and reject non-signed-wire project/manifest values. Strict Dart DTOs validate the real native wire/manifest ordering, seals, candidate/AssetStore closure, and closed status claims. The managed R3 session publishes stage/remove candidates by serialized full reopen, crash repair, exact-head byte CAS, and full published reopen; listing is exact-head read-only, races do not clobber, and integrity uncertainty poisons. The manifest persists no receipt bytes, local paths, or raw offsets; patched pair, exact USMAP, sidecars, and manifest are AssetStore/CAS objects. Results remain build-blocked, runtime-unqualified, artifact-not-granted, and native-publication-not-supported. Semantic schema/forms, preview/diff/undo, build/pack/deploy lowering, general package/export/name/reference/array/map/collection writing, and the sealed Unreal handoff remain missing. | Existing fixed-leaf runtime semantics are only as broad as their reviewed field proof. Managed project staging does not grant pack, deployment, or gameplay authority. Structural creation remains **Research-gated** until complete round-trip and runtime qualification. A stock Unreal Editor is not assumed to open cooked G1R packages or emit compatible output. |
...
Quest/NPC/DataAsset mutations without inventing build or runtime authority.
...
state while R3 is current. Bounded Quest/NPC Draft and verified DataAsset-stage
actions now share that R3 owner; this is a managed authoring shell, not yet a
unified semantic content-authoring flow.
...
Quest/NPC Draft and DataAsset-stage editing now use the managed session. General
semantic editing, migration orchestration into that session, clone/Save As,
