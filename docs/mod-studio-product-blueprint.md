...
| DataAssets and cooked content | A read-only DataAsset Lab opens a selected local `.uasset`/`.uexp` snapshot with its exact `.usmap`, separates walked/partial/unsupported exports, and lazily searches proven fixed-width leaves. It exposes no patch, project stage, save, pack, deploy, raw-offset, or Unreal-bridge control; native paths do not enter the evidence DTO, and a semantic browser/editor is still missing. | The native inspector and CLI share guarded no-follow reads, bounded Extract-v2/Patch-v2 validation, exact pair/USMAP/sidecar/generation proofs, and offset-free selectors. Native revision-3 authoring freshly reconstructs and union-probes the live target behind one opaque verified patch, binds the exact executable, rejects Store/game aliasing, and prepares a fully reopened immutable stage checkpoint. Closed bounded FFI prepare/list/remove routes now expose that boundary without receipt/path/offset leaks and reject non-signed-wire project/manifest values. The manifest persists no receipt bytes, local paths, or raw offsets; patched pair, exact USMAP, sidecars, and manifest are AssetStore/CAS objects. Exact-head list and registry-only remove fully reopen bounded historical bases. The result is deliberately unpublished, build-blocked, runtime-unqualified, artifact-not-granted, and publication-not-supported. Strict Dart/session integration, Studio stage/diff/undo, build/pack/deploy lowering, general package/export/name/reference/array/map/collection writing, and the sealed Unreal handoff remain missing. | Existing fixed-leaf runtime semantics are only as broad as their reviewed field proof. Native project staging does not grant pack, deployment, or gameplay authority. Structural creation remains **Research-gated** until complete round-trip and runtime qualification. A stock Unreal Editor is not assumed to open cooked G1R packages or emit compatible output. |
