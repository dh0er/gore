...
| Build and release | Studio can build/deploy represented legacy domains. Managed Mod Studio FFI/UI does not yet consume the compiler's typed install-restore status, and managed Quest/NPC compile, diagnostics, build, and deploy flows are not integrated. Project-wide semantic build roots, dependencies/rebase, immutable releases, and CI are also missing. | The bundle engine can build/reopen/inspect represented components. Separately, `gore-as` now exposes `InstallRestoreDisposition` on `CompileModuleReport` and `GameRunRegenReport`, independently from compile outcome and captured diagnostics. It distinguishes not started, restored exactly, unconfirmed process exit requiring recovery, and restore/finalization failure with retained recovery artifacts, so callers do not need to infer public cleanup state from compiler-error text. | Typed restore disposition is a compiler safety foundation, not Studio integration, managed Quest/NPC compiler support, or gameplay proof. End state adds deterministic profiles, semantic plans/conflicts, compatibility, rollback, provenance, closed-world release validation, and a recovery UI that consumes the structured state. |
