...
| Localization and spoken dialog | Localization and the compatibility existing-member replacement editor are integrated. Managed R3 Home now adds a guided dialog-line prerequisite V1 alongside bounded **Add Voice take**, **Manage Voice takes**, **Resolve Voice target**, and **Build Voice bundle** actions. A fresh managed project can create one project-owned localization and line plus an optional empty locale slot, or reuse one exact currently unused managed localization under revision binding. Reuse shows bounded friendly previews, distinguishes duplicate display names without exposing IDs, and offers only locales with non-whitespace text. The speaker input is explicitly only a label, not an NPC binding. This is not vanilla adoption. Import then searches an exact existing line, locale, and safe local Ogg; target resolution and offline build use a configured installation. Selection management does not: it shows the exact slot's authored candidates, never picks a default, and permits an explicit clear. All surfaces hide technical IDs/paths, recheck a fresh checkpoint, and refresh the visible revision/head after managed publication. This remains only a guided V1 plus bounded Voice tools, not a complete multilingual or dialog-production workspace. | Dialog-line creation/exact reuse is one exact-head/project/revision/target transaction. A separate read-only route double-reopens the exact fixed head and returns at most 1,000 sorted locale previews, each bounded to a UTF-8-safe 512-byte prefix; publication repeats this exact read. The prepare-only mutation FFI accepts no game root, fully reopens the immutable candidate with asset verification, checks the fixed head twice, and leaves guarded CAS, repair, and full published reopen to the managed session. It rejects a referenced, empty-locale, or stale reuse candidate and does not trust the unsealed global localization catalog. Take import performs semantic/capacity preflight, double-reads the Ogg before accepted CAS installation, and preserves localization plus alternatives. Selection is separately exact-bound; installed-target resolution seals executable/archive evidence and preserves ambiguity. The all-or-nothing Voice builder derives blockers from the complete graph and lowers only selected Approved Vorbis existing-member replacements into a deterministic format-3 offline bundle. | Dialog-line creation/exact reuse, preview, selection, target resolution, and managed project publication write neither the game nor a save. The new line has no topic, AngelScript, NPC/runtime registration, conditions, effects, or playable dialog path; it remains build-blocked and runtime-unqualified. The offline builder performs no deployment and proves no audible runtime behavior. Vanilla adoption needs a sealed generation-bound catalog. Managed deploy/undeploy, isolated testing, ambiguous-member choice, Voice-take preview/remove/unlink, recording/normalize/transcode/batch/coverage, qualified Opus output, and new-member runtime proof remain missing. End state adds complete multilingual production, history, review, release, and runtime qualification workflows. |
