...
| NPCs | In the visible schema-revision-2 Story flow, Mod Studio has a searchable, generation-bound, source-sealed NPC archetype catalog and picker. Creating from it atomically persists one offline NPC Draft plus deterministic `ScriptModule` through the managed session's guarded publication lane; the UI shows the saved Draft and generated source. Separately, native revision-3 authoring can atomically insert and fully reopen an unpublished NPC/module checkpoint against the exact current project. Managed R3 session publication and a unified semantic NPC inspector are not yet wired, and there is no production lowering, spawn, or runtime workflow. Every result remains build-blocked/runtime-unqualified. | The revision-3 transaction consumes freshly sealed Story+NPC catalog selection and a base-game-plus-exact-current collision inventory for modules, paths, symbols, and case-insensitive runtime IDs from the pinned catalog projection. It regenerates the existing NPC/Quest/module closure, preserves valid Quests, and fails closed on drift, residual ownership, or collisions. The FFI route rebuilds those inputs, revalidates game/head state, and prepares/reopens an immutable candidate without publishing the fixed head or writing game files. The deterministic three-class generator and retained Asghan-derived chain also compile, compose, reopen, and resolve offline. None of this proves build or spawn behavior. | New-class residence, effective visuals, distinct identity, conservative spawn, AI, dialog/quest separation, streaming, save/reload, persistence, and uninstall behavior are **separate NPC research gates**. Pinned catalog runtime IDs do not claim coverage of unknown game NPCs outside that projection. |
| Quests | Managed R3 Home now has a bounded friendly Quest Draft wizard for name, description, first objective, family, and giver. It uses freshly rebuilt catalog choices, derives technical identities internally from the exact checkpoint, rejects stale publication, and reloads the new revision/content view. It remains a one-shell Draft workflow; there is no complete semantic catalog, outline/state graph, conditions/effects, journal/reward editor, or production build/runtime workflow, and the separate visible managed Story flow remains schema revision 2. | The strict native `authoring_store_prepare_revision3_quest_draft_v3` FFI route derives fresh game/catalog and exact-current-project authority, runs the committed repeated-Quest C1 transaction and C2 Store persistence, and returns a fully reopened immutable checkpoint without replacing the fixed head. Closed raw transports, repeated source/project rechecks, working-store/game alias rejection, and signed-wire-safe values protect the boundary. The strict Dart wrapper validates the complete Quest/module/artifact closure and the managed session publishes by exact-head byte CAS, crash repair, and full reopen. Its structural result remains `blocked`, `runtime_unqualified`, `not_granted`, `fresh_capability_required`, and `not_supported`; a late race can leave only verified immutable CAS orphans. A retained probe independently showed two added quest subclasses instantiated as `Available` on one generation; that does not qualify generated transitions or effects. | Availability/start/success/failure transitions, dialog selection, journal, rewards, knowledge, effects, save/reload, persistence, and uninstall behavior are **separate quest research gates**. They do not become qualified when NPC spawning works. |
...
| DataAssets and cooked content | A read-only DataAsset Lab opens a selected local `.uasset`/`.uexp` snapshot with its exact `.usmap`, separates walked/partial/unsupported exports, and lazily searches proven fixed-width leaves. It exposes no project stage, semantic diff/undo, pack, deploy, raw-offset, or Unreal-bridge control; native paths do not enter the evidence DTO, and a semantic browser/editor is still missing. | The native inspector and CLI share guarded no-follow reads, bounded Extract-v2/Patch-v2 validation, exact pair/USMAP/sidecar/generation proofs, and offset-free selectors. Native revision-3 authoring freshly reconstructs and union-probes the live target behind one opaque verified patch, binds the exact executable, rejects Store/game aliasing, and prepares a fully reopened immutable stage checkpoint. Closed bounded FFI prepare/list/remove routes expose that boundary without receipt/path/offset leaks and reject non-signed-wire project/manifest values. Strict Dart DTOs validate the real native wire/manifest ordering, seals, candidate/AssetStore closure, and closed status claims. The managed R3 session publishes stage/remove candidates by serialized full reopen, crash repair, exact-head byte CAS, and full published reopen; listing is exact-head read-only, races do not clobber, and integrity uncertainty poisons. The manifest persists no receipt bytes, local paths, or raw offsets; patched pair, exact USMAP, sidecars, and manifest are AssetStore/CAS objects. Results remain build-blocked, runtime-unqualified, artifact-not-granted, and native-publication-not-supported. Studio stage/diff/undo, build/pack/deploy lowering, general package/export/name/reference/array/map/collection writing, and the sealed Unreal handoff remain missing. | Existing fixed-leaf runtime semantics are only as broad as their reviewed field proof. Managed project staging does not grant pack, deployment, or gameplay authority. Structural creation remains **Research-gated** until complete round-trip and runtime qualification. A stock Unreal Editor is not assumed to open cooked G1R packages or emit compatible output. |
...
state while R3 is current. This is an identity/read-only-content shell, not a
