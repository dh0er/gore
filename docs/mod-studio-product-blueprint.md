...
|---|---|---|---|
| Project and content library | The compatibility project remains a legacy format-1 bridge and the separate Story flow remains schema revision 2. The primary Project menu creates or opens one authoritative managed-R3 project. The canonical shell exposes Home, Content, Story, World, Localization & Voice, Validate & Test, Build & Release, History, and Settings & Expert. Content now contains a responsive `This mod` / `Base game` / `Installed` / `Search all` browser plus the separate verified-DataAsset-edits route. Scope pages mount lazily, retain state for the same project, and reset on project identity change. Project creation offers Empty, NPC Draft, and Quest Draft starters with the two-stage boundary visible before creation. Unsupported domains stay visible and honestly labelled. | Home and `This mod` read the exact current index. `Base game` derives curated NPC/Quest starting points from one generation-consistent evidence load and exposes broader NPC linkage only as search-gated inspect-only metadata. `Installed` is a query-first view over the exact managed package snapshot, capped at 100 rendered matches, and hands the selected canonical path to the existing exact inspector. `Search all` runs only for an explicit nonempty query, scans all three source projections independently in memory, retains at most 100 results per source, and reports each source's loading/complete/partial/error state separately. Its exact actions stay within the selected source: open a current mod identity, create a Draft from an exact Base catalog identity, or inspect an exact installed target. Every starter first creates/adopts canonical empty revision 0 through absent-head CAS; NPC/Quest then uses the existing guided exact-head publication as a separate revision. Cancel before publication retains the exact empty project, while an uncertain publication enters `requiresReopen` and permits bounded same-lock recovery or close/open. Story, Voice, Validate, the Voice-only offline builder, bounded History/Undo, and Settings remain as previously bounded. | Browsing, searching, and starter selection write neither the game installation nor a save and grant no deployment or runtime authority. The three search sources do not form one atomic combined snapshot; results grant no dependency, reference, backlink, or cross-source mutation authority. Installed rows are discovery metadata, not edit authority; Base-game Draft actions remain build-blocked/runtime-unqualified. General editors, migration/import/clone/Save As, named checkpoints, semantic history diffs, and recovery beyond the bounded timeline/`requiresReopen` repair, dependencies, references, indexed/virtualized large-scale search, collections, reusable/multi-domain templates, unified transactions, blob ownership, full build/deploy, and runtime qualification remain missing. |
...
| Localization and spoken dialog | The compatibility existing-member replacement editor remains integrated as a separate path. Managed R3 **Localization & Voice** is now a direct responsive workspace: authors search friendly project texts, edit the complete multilingual text map inline, add/remove safe locales, see shared line/speaker backlinks, and continue into the guided **New dialog line**, **Add Voice take**, **Manage Voice takes**, and **Resolve Voice target** workflows without a card-only landing page. A selected Quest transcript row can jump into this same workspace and exact line/language; the target is re-read against the matching project revision, head lifecycle token, opaque localization key, and DialogLine backlink. Technical IDs/LocIDs stay hidden. Manage previews the exact managed Ogg in-app with Play/Pause, Replay, seeking, progress, and Stop; it also edits status/selection and removes one take from one exact line/language with a destructive confirmation; selected removal clears atomically and never chooses a replacement. VoiceSlot locales cannot be removed or blanked; transcripts with retained candidate takes are locked. Unsaved localization text blocks Quest transcript mutations. A fresh project can create one project-owned localization and line plus an optional empty locale slot, or reuse one exact currently unused managed localization from bounded friendly previews. The speaker input remains only a label, not an NPC binding, and none of this adopts vanilla content. The Voice-only offline builder remains a separate bounded action. | Full-text editing first reads one exact project-owned `origin=new` LocalizationEntry under head/project/revision/target/entity-revision/LocID binding. Its pure transaction may change only the project revision, localization revision/text map, and union of global authoring locales. It rejects no-op/blank/limit/canonicalization errors, unrelated deltas, VoiceSlot removal/blanking, and candidate-backed transcript changes. The prepare-only FFI accepts no game root, fully reopens the immutable candidate, repeats fixed-head guards, and leaves guarded CAS, repair, and full published reopen to the serialized managed session. Dialog-line create/reuse retains its separate double-reopened 512-byte preview route and exact transaction; the Quest compound path embeds it and still publishes exactly one new project revision. Take import, status, selection, slot-scoped removal, target resolution, exact-current managed-CAS preview, and the deterministic format-3 Voice bundle keep independently bounded evidence. Preview alone traverses line/localization/slot/take/asset revisions, fully verifies the selected CAS Ogg, materializes one fixed leaf through a natively retained opaque cleanup capability, and unloads playback before token-bound non-recursive cleanup; it writes no Store or project state and exposes no CAS path. Removal retains shared take entities, removes an entity only after its final permitted use, and preserves the entire AssetStore and physical Ogg CAS. | Project-text editing and line/Voice project publication write neither the game nor a save and grant no topic, AngelScript, NPC/runtime binding, build, deployment, or gameplay authority. The new line still has no playable dialog path and remains build-blocked/runtime-unqualified; the offline Voice builder performs no deployment and proves no audible runtime behavior. Vanilla adoption needs a sealed generation-bound catalog. Localization delete/clone, broader line/slot relationship editing, bulk translation/XLIFF/review/history/rebase, complete conversation graphs, managed deploy/undeploy, isolated testing, recording/normalize/transcode, Opus build/runtime qualification, and new-member runtime proof remain missing. |
...
`Revision3ContentIndex`, shows friendly project metadata, Draft and readiness
status, domain/reference counts, and groups the existing bounded actions. It
links to reviewed DataAsset work and gives a direct fix when the game is not
...
the command palette, and complete workspace chrome remain missing.
...
distinct from audio or runtime qualification. Separately, the exact-current
...
completion still needs preview and history, explicit ambiguity
...
Voice take preview is now integrated as a read-only in-app capability. Next,
...
2. **Complete the first non-technical Voice slice:** extend the landed import,
   ...
   sealed existing-member offline build with preview, explicit
