...
| Localization and spoken dialog | Localization and the compatibility existing-member replacement editor are integrated. Managed R3 Home now also exposes a bounded **Add Voice take** wizard when a Gothic 1 Remake installation is configured: the author searches an exact existing line by speaker/name/Loc ID, chooses a canonical locale and local Ogg, names the take, assigns its status, and may select it only when Approved. The configured game root is used only as a read-only forbidden Store-root boundary, not as a catalog/content input. The list renders at most 50 matches per query, no line is implicitly selected, existing slot/selection facts are visible, and replacing a selection requires confirmation. The normal wizard preserves dialog text and writes only the managed project. It reloads the exact content index before publication and refreshes the visible revision/head afterward. This is not yet a complete localization/Voice production workspace. | The exact-head transaction accepts only an existing `DialogLine` and `LocalizationEntry`, creates or updates an unresolved locale `VoiceSlot`, appends a content-addressed Ogg `VoiceTake`, retains multiple candidates/status, and fully reopens its candidate. Before opening the Store, native code resolves the configured game root only to reject Store aliasing with the installation; the selected Ogg is separately treated as a read-only safe-file input. Semantic and capacity preflight happen before CAS. Two complete non-publishing source reads must match before accepted bytes are installed, so a changed valid source does not leave an avoidable blob. The managed session then owns guarded fixed-head publication and full reopen. Existing archive replacement, target resolution, bundle construction, and the sealed Asghan/Viper probe remain separate compatibility evidence. | Managed-R3 results remain `blocked`, `runtime_unqualified`, `not_granted`, and native-publication `not_supported`. There is no game write, archive-target resolution, managed build/deploy, preview/remove/unlink, recording/transcode/batch/coverage, or new-member runtime proof. Existing-member and new-member behavior retain separate operation/version gates; offline persistence does not prove audible gameplay. End state adds the complete multilingual production and history workflow. |
...
| Quests | Managed R3 Home has a bounded friendly Quest Draft wizard for name, description, one through eight ordered objectives, family, and giver. Objectives can be added, removed, and reordered. The wizard uses freshly rebuilt catalog choices, derives technical identities from project ID, current revision, and authored intent, separately binds publication to the exact root/canonical head, rejects stale publication, and reloads the new revision/content view. Existing single-objective projects keep byte-compatible generator-v2 JSON/source; multi-objective projects use generator v3. The R3 Content Library now also exposes **Edit quest outline** for a selected existing Quest. That editor can change its library name, player-facing title, and text/order of its existing one through eight objectives. It cannot add/remove objectives or edit description, family/parent, giver, stable IDs, technical identity, ownership, provenance, or collision `ArtifactRef`. Both surfaces remain ordered Draft-outline tools, not a complete semantic state graph, conditions/effects, journal/reward editor, or production build/runtime workflow; the separate visible managed Story flow remains schema revision 2. | Quest creation uses the strict native `authoring_store_prepare_revision3_quest_draft_v3` FFI route with fresh game/catalog and exact-current-project authority. Existing-outline editing uses the separate pure count-preserving transaction and strict prepare-only `authoring_store_prepare_revision3_quest_outline_edit_v1` route: its closed payload has only the Store root plus exact canonical project/request transports and deliberately has no `game_root`. Native code binds the exact head/project/target/Quest, proves the existing Quest/module closure, performs full exact-current Store preflight, regenerates only outline-derived source, and fully reopens an immutable candidate without replacing the fixed head. Strict Dart validation covers the candidate; the managed session publishes by exact-head byte CAS, crash repair, and full reopen. Both paths remain `blocked`, `runtime_unqualified`, and native-publication `not_supported`; creation additionally grants no artifact authority and requires fresh source inspection. Neither path reads/writes a save or writes/launches the game. A retained probe showed two added quest subclasses instantiated as `Available` on one generation; it does not qualify objective order, generated transitions, or effects. | Availability/start/success/failure transitions, ordered-objective runtime enforcement, dialog selection, journal, rewards, knowledge, effects, save/reload, persistence, and uninstall behavior are **separate quest research gates**. They do not become qualified when NPC spawning works. |
...
state while R3 is current. Bounded Quest/NPC Draft, Voice-take, and verified
DataAsset actions now share that R3 owner; this is a managed authoring shell,
not yet a unified semantic content-authoring flow.
...
The first bounded line-centric Voice transaction/import slice now extends the
managed revision-3 session instead of adding another format-1 list or parallel
project state. It uses strict exact-head open/import, working-store AssetStore
blob I/O, one line/locale/slot/take transaction, guarded publication, repair,
and full reopen. Production completion still needs archive-target resolution,
deterministic export/build materialization, preview/remove/history, and the
remaining authoring tools; the landed persistence slice must not be presented
as that complete workflow.
...
Quest/NPC Draft, Voice-take, and DataAsset-stage editing now use the managed
session. General semantic editing, migration orchestration into that session,
clone/Save As, autosave/full history, and all-domain transactions remain
integration work; the visible schema-revision-2 Story flow keeps its narrower
scope.
...
1. **Finish the managed phase-one substrate:** the owned R3 working directory,
   ...
   project state.
2. **Complete the first non-technical Voice slice:** the landed Home wizard can
   find an exact existing dialog line, attach a localized Ogg take, retain
   multiple candidates/status, and publish the managed checkpoint. Add
   preview/remove, archive-target resolution, exact existing-member build
   lowering, save/reopen/history UX, and validation without archive terminology.
   Project persistence alone does not satisfy the production milestone.
