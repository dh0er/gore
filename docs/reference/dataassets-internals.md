# Cooked DataAsset internals

Implementation contracts and invariants for the `gore asset` commands and the
Mod Studio DataAsset surface — an engineering record, not instructions. The
user-facing workflow is [Cooked DataAssets](../guide/dataassets.md); section
numbers below mirror that guide's steps.
