...
project schema changes update that model, its reader/writer, tests, and UI in
lockstep. Studio must not carry project migration, compatibility readers,
converters, or old-format fallbacks. Snapshot V2 is only the portable
backup/restore format, not a second editable project format.
...
| Items and economy | Content -> Items now provides exact managed-R3 authoring for supported scalar fields on existing vanilla classes. Authors browse the familiar categorized catalog, add or remove typed field overrides, save one project revision, and revert an exact-current patched item to game defaults without seeing managed entity IDs or provenance. New-item identity, clone, and economy workflows are not present. | The native embedded 798-class catalog and its field schemas are sealed to the exact target and head. Its authority admits only registered audited game-generation executable seals and commits to the exact target, generation ID, and complete sorted `(class, field, type)` manifest; unknown targets and unreviewed manifest changes fail closed. Signed-32-bit-integer, finite-32-bit-float, and boolean `ItemPatch` create/update/remove operations use exact representation domains rather than guessed gameplay/economy bounds. They rebind Store root, project ID/revision/head, catalog layer/seal, source seal, current entity revision, and the exact delta inside the serialized publication lane, then use prepare-only, full candidate reopen, fixed-head CAS, and full published reopen. An existing patch whose catalog layer, class source seal, supported fields, types, or domains no longer match is invalid current input; the unreleased product provides no legacy revert or migration route. Status remains build-blocked/runtime-unqualified/publication-not-supported at the native boundary. | Project edits write neither the game nor a save. Item bundle lowering, deployment, runtime behavior, new identity, construction, visuals, equip/use behavior, trade/loot integration, and persistence are **Research-gated** independently. |
...
| Build and release | **Test & Release** now starts with an exact-current project build preview over eight fixed domains. It shows empty, blocked, and semantically covered states, distinguishes author work from missing toolkit support, and keeps technical seals collapsed by default. This preview is not a build button: playable build and deployment remain blocked unless each receives exact checkpoint evidence and its own connected action; Script evidence never unlocks either one, and build evidence never unlocks deployment. The current bounded Voice readiness/offline-build continuation remains available in place. | Native code derives a deterministic, path-free plan from the complete reopened managed-R3 project plus its complete verified DataAsset stage set. The strict app wire independently checks project counts, Voice expectations, stage manifests, canonical blocker ordering, and the input/result seals. Windows retains the complete no-follow Store path chain with rename-blocking handles; Linux additionally latches transient path replacement through exact-FD inotify watches. Other Unix targets fail this capability closed until they have an equivalent primitive. Existing bundle, deployment, compiler-report, Voice-bundle, and reviewed DataAsset-pack components retain their separate bounded evidence. No project-preview result returns source, paths, artifacts, targets, or an executable capability. | Deterministic all-domain lowering, dependency-derived roots, deploy/undeploy, isolated launch/postflight, rollback, immutable releases, provenance, and CI remain missing. The preview creates nothing and explicitly grants no build, install, deployment, publication, runtime, game, or save authority. Unsupported compiler-hook signatures continue through the normal compiler fallback and do not grant gameplay proof. |
...
For the two exact-current read-only planning routes, Linux also binds every
opened Store file to the mount ID captured from the retained root handle before
reading or hashing it. This closes transient bind/FUSE-mount substitution in
addition to the exact-FD inotify path-change latch.

The app independently checks canonical project counts, Voice expectations,
stage-manifest membership/seals, blocker structure, and the envelope seals. It
does not duplicate the native reviewed cooked-DataAsset evaluator; DataAsset
ready/blocked classification is therefore native evidence from the exact
reopened manifest set, not an independent Dart re-evaluation.
