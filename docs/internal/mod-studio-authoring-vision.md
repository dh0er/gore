...
- [GORE Mod Studio today](../../apps/mod-studio/README.md)
- [Bundle and deployment contract](../guide/bundles.md)
- [AngelScript dialog authoring and live proof](../guide/dialog-authoring.md)
- [AngelScript quest authoring and discovery proof](../guide/studio-quest.md)
- [AngelScript logical NPC authoring](../guide/studio-npc.md)
- [Managed revision-3 Voice authoring](../guide/studio-voice.md)
- [Cooked DataAsset fixed-leaf workflow](../guide/dataassets.md)
- [Offline AngelScript default patching](../guide/angelscript-defaults.md)
- [Managed project snapshot export](../guide/studio-project-export.md)
- [Managed project snapshot import V2](../guide/studio-project-import.md)
...
the current model, reader, writer, tests, and UI. There are no legacy projects,
so Mod Studio carries no compatibility readers, migration/conversion lanes, or
old-format fallback. The standalone-tool surface is
...
- **Test & Release** now presents six independently truthful areas: project
  ...
  evidence and an executable action; one cannot authorize the other. The
  Scripts card now runs one exact project-wide compiler check over every
  managed Quest/NPC `ScriptModule` in a single shared source tree. Native code
  regenerates the complete module graph from the Store, seals the game inputs
  and deterministic module manifest, restores the installation, destroys the
  private regenerated cache, and completes its Store/game closing audit under
  the same install guard. The returned closing evidence keeps Store and game
  exactness, drift, inspection failure, and a not-run preflight state separate;
  Store uncertainty requires a full project reopen while game-only uncertainty
  remains independently retryable. The app also recomputes and byte-binds the
  complete sorted module-manifest seal from its exact canonical project instead
  of accepting counts alone. The UI retains only bounded diagnostics and
  checkpoint-scoped Script evidence; it grants no build, deployment, runtime,
  publication, or reusable-artifact authority.
- At the top of the Test & Release **Checks** section, an exact-current
  **Project build preview** now classifies the complete managed-R3 checkpoint
  across eight
  fixed content domains: localization, dialog, Voice, NPCs, Quests, scripts,
  items, and verified DataAsset stages. It separates blockers the author can
  resolve from missing toolkit lowering, shows deterministic input/result seals
  only in technical details, and discards late results after checkpoint drift.
  Native code reopens the Store and every complete DataAsset stage before and
  after planning; the app independently recomputes project counts, blocker
  ordering, Voice expectations, stage-manifest membership/seals, and both
  envelope seals. DataAsset ready/blocked classification remains native because
  Dart deliberately does not duplicate the reviewed cooked-asset evaluator.
  This is a path-free, evidence-only readiness inventory: it creates no
  artifact, accepts no output
  or game path, and grants no build, install, deploy, publication, runtime,
  game, or save authority. Playable build and deployment therefore remain
  disabled.
...
- The project-wide check is deliberately not an aggregation of those
  per-entity checks. It emits all base and project modules together and invokes
  the game compiler at most once, so mutual project references, global symbol
  collisions, and cross-module failures are observed in their real shared
  namespace. Add-only namespace/path collisions, Windows case-fold collisions,
  unexpected regenerated modules, missing modules, unusable diagnostics,
  cleanup uncertainty, closing drift, and guard-release failure all fail
  closed. A clean empty project reports `not_needed_empty` without launching
  the compiler. Compiler rejection is useful author feedback, not a build plan.
- The selected managed-R3 Quest/NPC detail is one responsive, exact-current
  ...
  & Voice**, **References**, and **Problems & Checks**. NPC likewise exposes
  **Profile**, **Dialog & Voice**, **References**, and **Problems & Checks**.
  Unmodeled NPC areas remain discoverable in one initially collapsed, honest
  summary instead of occupying empty primary tabs. Journey
...
- The exact-current Quest Journey now carries a persistent **Write this
  ...
  second publication. Every accepted Quest follows the same generator-V4-native
  path with no alternate behavior-review branch. Progress is derived from the
  same reopened Journey projection, not
...
- The primary managed-R3 **Story** destination is now a direct authoring
  ...
  mutation, build, deploy, runtime, game, or save authority. The useful classic
  standalone tools remain the UX baseline and own no project/session state.
...
- Managed R3 **Test & Release** now hosts bounded **Problems & Readiness V1**.
  ...
  integrated. Its Problems assessments remain independent: reference
  integrity, DataAsset registry, and game configuration never infer Script
  readiness. The adjacent Scripts card now owns the exact project-wide
  evidence-only compiler result described above; managed build remains
  **Blocked** and runtime remains **Unqualified**. This is not a complete
  incremental validator, quick-fix system, build plan, deployment result, or
  runtime proof.
...
- **Project > Close** now closes the current coordinator-owned managed-R3
  session after confirming any unsaved visible editor draft. It neither deletes
  project data nor supplies Save As, history, or recovery.
...
| Items/world/spawns | **Existing-item scalar project authoring integrated; new identity/world missing** | Supported existing signed-32-bit-integer, finite-32-bit-float, and boolean overrides can be created, updated, and reverted as exact-current managed `ItemPatch` entities. These are representation bounds, not guessed gameplay/economy rules. The native catalog, audited game-generation registry, complete class/field/type manifest, Store root, target executable, project ID/revision/head, catalog/source seals, schema, provenance, entity delta, and publication receipt are independently rebound before a project revision advances. Item bundle lowering and runtime behavior remain blocked. New item identity, placed world actors, spawn points, routines, level edits, and world-partition integration have no semantic Studio workflow or production proof. An optional sealed Unreal handoff is planned only for explicitly supported future operations; no bridge or compatible world writer exists today. |
...
| Build/deploy/undeploy | **Bounded managed-R3 Voice and reviewed-DataAsset offline lowerers proven; managed deployment and project-wide build missing** | The safe bundle/deployment engine exists underneath the Studio, but no alternate project backend owns it. Existing-member spoken-line replacement remains bounded evidence until it is driven from the managed project. R3 NPC/Quest Drafts and generic DataAsset stages remain outside a production build. The project-wide Script check proves only compiler acceptance of one exact shared module tree; its regenerated cache is neutralized and cannot become a build input. Managed Voice has an explicitly labelled all-or-nothing offline builder for verified selected Vorbis existing-member replacements; one exact-current reviewed Human/Scavenger/Wolf `FeetTextureSize` stage can separately emit a strictly reopened offline package triplet. Neither path deploys or grants project-wide pack/gameplay authority. Project-wide semantic roots, dependency/risk review, managed deploy/undeploy, rollback, and isolated runtime qualification remain missing. |
| Validation | **Bounded Test & Release workspace and project-wide Script compiler evidence integrated; full validator missing** | Test & Release exposes six separate areas: project structure, scripts, Voice, DataAssets, playable build, and deployment. It rehosts the current Problems & Readiness V1 view and Voice readiness/build continuation without collapsing them into an overall Ready result. The Scripts card regenerates every managed Quest/NPC module from one exact Store snapshot, compiles all modules together in at most one guarded game-compiler run, verifies the regenerated module manifest, discards the private cache, and retains bounded diagnostics only after exact restore and a same-guard Store/game closing audit. The response preserves separate Store/game closing dispositions; the app independently derives the same sorted manifest seal from the canonical project and requires an exact match. Empty, rejected, stale, preflight-blocked, and recovery-required outcomes remain distinct. Store closing drift or inspection uncertainty forces a full project reopen; game-only uncertainty does not masquerade as Store integrity loss. Every evaluated result must carry Script-only evidence matching the current project ID, revision, and canonical head; drift clears it immediately. Playable build and deployment receive neither evidence nor callbacks from this check. Project-wide incremental semantic validation, transactional quick fixes, general build planning, managed deployment, and runtime qualification remain missing. |
...
export](../guide/studio-project-export.md) and [Managed project snapshot import
V2](../guide/studio-project-import.md).
...
Available/Start/Success/Failure and keeps each dialog row beside its stable V4
objective. Non-V4 Quest content is invalid input rather than a compatibility or
migration case. Journey hands off to the
...
- current project schema version, changed in lockstep across the sole R3 model;
...
## 5. Managed R3 project model

`ProjectRevision3` is the sole durable authoring model. It is not
`gore_mod::BuildSpec` with extra UI fields. `BuildSpec` remains a compact,
declarative deployment intermediate representation; the managed graph retains
author intent, origins, dependencies, and reusable source assets. Checkpoint
history belongs to the authenticated Store snapshot, not a second project
schema.
...
ProjectRevision3 {
  ...
  authoring_locales: ordered set<LocaleCode>,
...
playable dialog. Every accepted Quest is generator-V4-native and carries one
required closed stable-slot plan. Reads and derivations reject non-V4 Quest
data; outline and context edits preserve the current plan, while an explicit
  behavior edit replaces only that current V4 plan. Non-V4 Quest data is invalid
  input. The transaction remains prepare-only at
...
Phase 6 promotes only the Draft quest templates whose required mechanisms have
passed those gates.
...
Promote only qualified Draft quests to production-capable new quests, then
...
5. The landed bounded V4 fixture round-trips stable objective slots/order, all
   ...
   deterministic source. Non-V4 Quest content is rejected rather than upgraded
   or preserved through a compatibility path. The sealed 1.0.3 compiler
   qualifier reopens/disassembles the exact listed
