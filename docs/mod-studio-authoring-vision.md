...
- Managed Voice entry actions now fail closed against the exact-current
  projection. Add, Manage, and Resolve require at least one intact
  Voice-authorable `DialogLine` with a resolved same-project
  `LocalizationEntry`; Add and Resolve also require a configured installation.
  Stale or unavailable projections cannot re-enable them. This exposes the
  prerequisite but does not yet create or adopt the missing line/localization
  pair for a fresh project.
...
checkpoint. The immediate queue is: (1) complete the existing-line,
localization, and Voice offline journey, including create/adopt and reopen;
(2) build, reopen, and verify one reviewed managed DataAsset stage; (3) finish
project deletion, undo/history, export, and recovery, with Close already
landed; (4) make the existing NPC and Quest surfaces coherent authoring
journeys; and (5) complete honest managed build/release and qualified test
paths. Broad World or level authoring remains frozen until these workflows are
usable end to end. World may stay visible as an unavailable destination, but
research or a placeholder must not displace this queue.
...
The current fail-closed prerequisite gate is necessary but does not complete
this phase: a fresh managed-R3 project still lacks an author-facing way to
create or adopt the required `DialogLine`/`LocalizationEntry` pair.
