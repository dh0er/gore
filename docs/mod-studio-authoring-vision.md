...
|---|---|---|
| Project save/load | **Partial Studio paths; safe revision-3 managed session proven** | The normal app session still saves `.goremod` format 1 and has no typed cross-domain graph or durable history. Separately, the visible Story create/open flow owns an exclusive schema-revision-2 managed working directory. A dedicated revision-3 managed-session API now reuses its safety core: exclusive lock, serialized derive/save, immutable native checkpoint preparation, full candidate verification, exact fixed-head byte CAS, crash-repair journal, poison-on-uncertain publication, and full published reopen. `gore-authoring` has closed revision-1/2/3 documents, explicit revision-2-to-3 migration, and an immutable working store with sealed heads, snapshot/entity shards, and physical Ogg/Quest-artifact CAS blobs. The R3 session exposes only durable project/checkpoint identity and is not yet the unified normal-app project, launched UI flow, migration/history system, or all-domain transaction owner. |
...
| Quest inspection/edit/create | **Missing semantic Studio path; revision-3 offline Draft pipeline** | Mod Studio still has no semantic quest catalog, graph, or friendly revision-3 managed wizard; the existing visible managed Story flow remains schema revision 2. Revision 3 stores compact Quest collision-artifact references, imports and reopens the exact artifact plus old basis snapshot, atomically inserts one offline Quest Draft with its deterministic `ScriptModule`, and regenerates it for source inspection only after a fresh source-bound capability verifies retained identities. The dedicated revision-3 Store/FFI/Studio bridge and managed session can open, prepare, verified-CAS publish, repair, and reopen checkpoints, but no Quest transaction or UI uses that session yet. Every generated outcome remains build-blocked/runtime-unqualified. A retained report of two added `UQuest` instances as `Available` proves only narrow discovery/instantiation on one generation, not transitions, dialog selection, effects, rewards, or persistence. See [quest authoring](quest-authoring.md). |
