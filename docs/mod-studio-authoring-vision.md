...
- The landed Story Workbench V1 checkpoint replaces the selected
  managed-R3 Quest/NPC tools menu with one responsive, exact-current content
  detail surface. Quest exposes **Overview**, **Story**, **Logic**, **Dialog &
  Voice**, **References**, and **Problems & Checks**; NPC exposes **Profile**,
  **Story**, **Routine**, **Inventory**, **Dialog & Voice**, **References**, and
  **Problems & Checks**. The existing Quest outline/context/transition editors
  and Quest/NPC source inspections remain the only actionable operations;
...
a revision advance of the same project only while that entity still exists.
Project identity changes clear both, and a removed/non-story entity cannot
retain stale tab state. **References** shows only exact projected outgoing
entity/asset links and derived incoming links from the current index. **Problems
& Checks** counts only unresolved projected links and reuses the bounded
inspection action; it is not a project-wide validator or a build/runtime
readiness claim.
...
| Existing cooked DataAsset fixed leaves | **Managed revision-3 typed editor, first reviewed Footstep form, and verified registry proven; build missing** | Receipt-bound extract/patch/re-inspect/offline-pack and the generic inspector-proven Bool/integer/float/color/vector editor remain available. The normal installed-package route instead binds a sealed ordinal and independently reconstructs its package/USMAP evidence before staging. Its first closed reviewed schema recognizes only the exact Human, Scavenger, and Wolf `FootstepTag` targets and exactly one `BoneData/BoneFeetData/FeetTextureSize` Vector4 leaf. Studio exposes a friendly X/Y form with scale presets, Before/After preview, and preserved Z/W; values are explicitly raw asset units. The reviewed FFI request carries only head, ordinal, package/source seals, schema/field, and X/Y—no target, selector, USMAP, bytes, offsets, receipt, output, build, or deployment authority. Native code rediscovers and lowers the leaf through two complete drift windows; Dart independently recomputes the reviewed and ordinary stage bindings before the managed exact-head publication lane may advance. This is not a general schema editor and grants no build, pack, deploy, gameplay, runtime, or future-reinspection authority. Broader reviewed schemas, gameplay-qualified units, multi-edit/undo, build lowering, post-pack verification, structural editing, new assets, and the sealed Unreal handoff remain missing. It writes only the managed project, never the game installation or a save file. See [DataAsset authoring](dataasset-authoring.md). |
...
Profile and Problems reuse the existing exact-current profile/source inspection;
Story, Routine, Inventory, and Dialog & Voice stay visible but explicitly
unavailable because the Draft schema does not model them. Outgoing and incoming
links come only from the current `Revision3ContentIndex`, while the problem
count covers only unresolved projected links. The three separate Draft-only,
build-blocked, and runtime-unqualified badges therefore remain mandatory.
...
compiler inspection. Dialog & Voice remains explicitly unavailable, and the
reference projection is not build or runtime readiness. The lifecycle action
itself is one friendly behavior table over the fixed root/objective edges, with
a sequential template and bounded typed condition/action dialogs. It shares the
exact project/session publication lane and generates deterministic V4
AngelScript, but it has no synchronized transcript/general graph, journal,
reward, item, raw-source, complete simulation, build, deploy, or runtime-test
surface. The Studio labels the saved checkpoint Draft-only, build-blocked, and
runtime-unqualified.
