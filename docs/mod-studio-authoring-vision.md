...
| Quest inspection/edit/create | **Managed revision-3 ordered-outline Draft creation and count-preserving existing-Quest outline editing proven; complete semantic editor missing** | Managed R3 Home exposes a bounded Quest Draft wizard for name, description, one through eight ordered objectives, Quest family, and giver. Objectives can be added, removed, and reordered; friendly Story-catalog choices are rebuilt at open and immediately before publication. The UI hides generated IDs/namespaces/paths, derives technical identities from project ID, current revision, and authored intent, separately binds publication to the exact managed root/canonical head, rejects stale-wizard publication, and refreshes the revision/content library after success. Existing single-objective R3 entities retain project generator version 2 with byte-identical canonical JSON and source; multi-objective entities use version 3 and regenerate the complete ordered source. In addition, selecting an existing Quest in the R3 Content Library now exposes **Edit quest outline**. It can change the library name, player-facing title, and text/order of the existing one through eight objectives, but cannot change objective count, description, family/parent, giver, stable IDs, technical identity, ownership, provenance, or collision `ArtifactRef`. The pure edit transaction proves and regenerates the exact owned module while preserving technical identity. Its strict native `authoring_store_prepare_revision3_quest_outline_edit_v1` route accepts no `game_root`, fully binds and preflights the exact current Store/project/target/Quest, and prepares plus fully reopens only an unpublished immutable checkpoint. Quest creation separately uses fresh game/catalog authority through `authoring_store_prepare_revision3_quest_draft_v3`. Neither native route replaces the fixed head; strict Dart validation and the managed session publish only by guarded exact-head byte CAS, repair journal, and full published reopen. Both operations remain `blocked`, `runtime_unqualified`, and native-publication `not_supported`; Draft creation additionally grants no artifact authority and requires fresh source inspection. Objective order is persisted source shape, not proof that runtime transitions enforce it. The semantic Quest state graph, conditions/effects, journal/reward authoring, build lowering, and runtime workflow remain missing. The retained `Available` instances prove only narrow discovery on one generation, not transitions, dialog selection, effects, rewards, or persistence. Neither path writes the game installation or a save file. See [quest authoring](quest-authoring.md). |
| Voice archive editing | **Compatibility existing-member slice proven; bounded managed revision-3 transaction/import slice proven; production workflow missing** | The compatibility line-first editor still resolves installed language archives beside dialog text, validates and previews/removes a chosen Ogg, persists it in the portable project, and includes exact existing-member replacement in Build/Deploy. Separately, managed R3 Home exposes **Add Voice take** only when a Gothic 1 Remake installation is configured. That root is a read-only forbidden Store-root boundary used to reject a project Store that aliases the installation, not a catalog/content input; the selected Ogg is separately treated as a read-only safe-file input. Its bounded, search-first wizard selects one exact existing `DialogLine`, a canonical locale, one real Vorbis/Opus Ogg, a take name and status; it creates or updates that line's unresolved locale `VoiceSlot`, appends a `VoiceTake`, and permits selection only for `Approved`. Existing localization text is preserved by the normal wizard, and replacing an already selected take requires explicit confirmation. The service reloads the exact content index before publication; the coordinator binds root/project/revision/canonical head and refreshes the visible checkpoint after guarded managed-session publication. Before Store open, native preparation checks Store/game-root disjointness; it then performs semantic and capacity preflight before Ogg CAS, reads and validates the source twice without Store mutation to reject a changed file, installs only the accepted bytes, and fully reopens the unpublished candidate. Results remain `blocked`, `runtime_unqualified`, `not_granted`, and `not_supported`: there is no game write, archive-target resolution, managed Build/Deploy lowering, preview/remove/unlink, recording/transcode/batch/coverage workflow, new-member runtime proof, or save write. See [Voice authoring](voice-authoring.md). |
...
| Existing cooked DataAsset fixed leaves | **Managed revision-3 first typed fixed-width editor and verified registry proven; build missing** | Extract, same-width compare-and-swap patch, re-inspect, and offline pack remain receipt-bound. The first guided semantic slice edits only inspector-proven `editable=true` Bool/integer/float/color/vector leaves and still requires a separately produced, exact ExtractReceipt-v2. A native read-only route verifies that receipt and returns only its `/Game` target, package/USMAP seals, and lengths; Studio matches those facts to the inspection and requires explicit target confirmation before preview because byte-identical packages can have different targets. The prepare-only route performs authoritative typed encoding, recreates and fully verifies a private PatchReceipt chain, and returns an intent-binding SHA-256 over confirmed target, canonical offset-free selector, and encoded replacement. Strict Dart code independently binds the same intent and accepts only the exact stage/target/candidate closure. The managed R3 session publishes through serialized full reopen, crash repair, exact fixed-head byte CAS, and full published reopen. Home exposes the guided summary/confirmation/Before-After flow alongside expert PatchReceipt import, search, verified facts, and confirmed registry-only removal, all bound to the exact root/project/revision/head. The DataAsset Lab itself remains read-only. This is not a general schema editor and grants no build, pack, deploy, gameplay, runtime, or future-reinspection authority; reviewed gameplay schemas/units, multi-edit/undo, build lowering, post-pack verification, structural editing, new assets, and the sealed Unreal handoff remain missing. It writes only the managed project, never the game installation or a save file. See [DataAsset authoring](dataasset-authoring.md). |
