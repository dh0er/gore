...
| Existing cooked DataAsset fixed leaves | **Managed revision-3 staging and visible verified-edit registry proven; semantic editing/build missing** | Extract, same-width compare-and-swap patch, re-inspect, and offline pack remain receipt-bound. Extract-v2/Patch-v2 wires, bounded readers, pair/USMAP/sidecar/generation validators, and opaque verified capabilities live in one shared `gore-asset` workflow used by the CLI. Native revision-3 authoring consumes that opaque input, freshly reconstructs and union-probes the live target, binds the exact non-empty executable, rejects Store/game aliasing, and prepares a fully reopened immutable candidate containing only a closed path-free stage manifest plus patched pair, exact USMAP, and sidecars in AssetStore/CAS. Closed bounded FFI routes expose exact-head prepare/list/registry-only remove, reject unsafe receipt inputs and non-signed-wire nested values, and return no path, receipt, raw offset, artifact, build, runtime, pack, deployment, or native-publication authority. Strict Dart DTOs validate real native wire/manifest ordering, nested seals, complete candidate/AssetStore bindings, and closed statuses. The managed R3 session publishes prepare/remove by serialized full reopen, crash repair, exact fixed-head byte CAS, and full published reopen; listing is an exact-head read, drift cannot clobber, and integrity uncertainty poisons. Home now has a searchable **Verified DataAsset edits** view that imports a PatchReceipt through a file picker, lists friendly verified facts, and performs confirmed registry-only removal against the exact root/project/revision/head. The separate DataAsset Lab remains read-only. Neither surface authors semantic values directly or grants build, pack, deploy, gameplay, runtime, or future-reinspection authority. Semantic schema/forms, preview/diff/undo, build lowering, post-pack verification, structural editing, and the sealed Unreal handoff remain missing. See [DataAsset authoring](dataasset-authoring.md). |
