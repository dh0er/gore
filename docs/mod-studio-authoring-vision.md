...
|---|---|---|
| Project save/load | **Partial Studio paths; safe revision-3 session and current-project coordinator foundations proven** | The normal app session still saves `.goremod` format 1 and has no typed cross-domain graph or durable history. Separately, the visible Story create/open flow owns an exclusive schema-revision-2 managed working directory. A dedicated revision-3 managed-session API reuses its safety core: exclusive lock, serialized derive/save, immutable native checkpoint preparation, full candidate verification, exact fixed-head byte CAS, crash-repair journal, poison-on-uncertain publication, and full published reopen. A typed app-wide current-project coordinator foundation owns at most one authoritative current Legacy format-1 or managed revision-3 lease; a candidate lease may coexist only while it is fully validated before adoption. It serializes open/adopt/save/verify/close, preserves the current lease when a candidate open fails, retires leases once, retains terminal cleanup failures only as diagnostics, and maps managed Save to exact-head full-reopen verification without preparing or publishing a checkpoint. `gore-authoring` has closed revision-1/2/3 documents, explicit revision-2-to-3 migration, and an immutable working store with sealed heads, snapshot/entity shards, and physical Ogg/Quest-artifact CAS blobs. The coordinator/provider is not yet wired into Home, menus, `Ctrl+S`, or app-shell lifecycle; the R3 session exposes only durable project/checkpoint identity and is not yet the launched normal-app project, migration/history system, or all-domain transaction owner. |
...
| New NPC identity | **Offline compile/compose and managed revision-2 Draft persistence proven; runtime missing** | A new `CharacterDefinition` with a new `UniqueName`, a linked `AIAgentConfig`, and a linked `SpawnAIAgentDefinition` compile and compose as one additive AngelScript module while leaving visual/actor defaults inherited from Asghan-derived parents. In the visible Story flow, the source-sealed picker drives the native story transaction, which atomically persists one schema-revision-2 NPC Draft plus deterministic `ScriptModule` through the managed session's serialized derive/save lane, exact-head CAS publication, and full reopen. A safe revision-3 session foundation now exists separately, but NPC insertion is not wired into it. The saved R2 result remains build-blocked/runtime-unqualified, with no production lowerer, spawn, or runtime workflow. Class residence, discovery, effective visuals, distinct identity, spawning, dialog/quest separation, and persistence are not proven. Spawning another body for an existing identity remains a different, weaker proof because it shares that identity's dialog/quest state. Cooked-asset creation is required for genuinely new visual/content assets and for any registry or collection change the recovered chain actually requires, but is not currently proven mandatory for a logical NPC identity. See [NPC authoring](npc-authoring.md). |
...
| Quest inspection/edit/create | **Missing semantic Studio path; repeated revision-3 offline Draft and Store preparation proven** | Mod Studio still has no semantic quest catalog, graph, or friendly revision-3 managed wizard; the existing visible managed Story flow remains schema revision 2. The earlier revision-3 path stores compact Quest collision-artifact references, imports and reopens the exact artifact plus old basis snapshot, persists a first offline Quest Draft with its deterministic `ScriptModule`, and regenerates source only after a fresh source-bound capability verifies retained identities. A version-2 collision artifact binds the base inventory, trusted Story catalog, exact project identity/revision/target/head and project bytes, non-Quest state, prior-Quest evidence, and all collision domains; structurally reopening it never recreates authority. A linear filesystem-free transaction consumes the fresh prepared capability plus exact canonical request/current-project transports, checks Base Game and exact-current-project collisions, resolves trusted parent/giver catalog IDs, constructs the next `QuestDraft` and deterministic `ScriptModule`, increments the revision, and returns an externally opaque result only after canonical reopen equality. The lower Store imports the exact artifact through bounded, no-clobber CAS with full raw verification against the unchanged basis head. The consuming Story persistence helper then revalidates the canonical candidate, artifact, exact basis, entity/asset delta, QuestRef, and regenerated module before preparing and fully reopening the checkpoint. It never publishes the fixed head; a late race can leave only verified immutable CAS orphans, never a published candidate head. FFI orchestration, managed-session publication, and Studio integration remain missing. Every generated outcome remains build-blocked/runtime-unqualified, grants no artifact authority, and requires a fresh capability for source inspection. A retained report of two added `UQuest` instances as `Available` proves only narrow discovery/instantiation on one generation, not transitions, dialog selection, effects, rewards, or persistence. See [quest authoring](quest-authoring.md). |
...
| Existing cooked DataAsset fixed leaves | **Proven narrow offline write path; shared verified receipts; read-only Studio inspection** | Extract, same-width compare-and-swap patch, re-inspect, and offline pack remain receipt-bound. Extract-v2/Patch-v2 wires, bounded readers, pair/USMAP/sidecar/generation validators, and opaque verified-file capabilities now live in one shared `gore-asset` workflow used by the CLI; full normalized receipt bytes are frozen by golden hashes. The read-only DataAsset Lab can safely open a selected local `.uasset`/`.uexp` pair with its exact `.usmap`, separate walked/partial/unsupported exports, and lazily search offset-free proven leaves under native allocation/work/hash/depth/count budgets and exact seals. The shared workflow is foundation for a future project/FFI transaction, not managed staging: Studio still exposes no patch, stage, save, pack, deploy, raw-offset, or semantic gameplay control. See [DataAsset authoring](dataasset-authoring.md). |
