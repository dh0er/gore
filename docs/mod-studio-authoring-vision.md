...
|---|---|---|
| Project save/load | **Partial Studio paths; app-wide revision-3 ownership shell proven** | The compatibility session still saves `.goremod` format 1, and the separate visible Story create/open flow still owns a schema-revision-2 managed working directory. A dedicated revision-3 managed session reuses the proven exclusive-lock, serialized derive/save, immutable checkpoint, full-reopen, exact fixed-head CAS, repair-journal, and poison-on-uncertain-publication safety core. The typed current-project coordinator now drives Home, the Project menu, and `Ctrl+S`: it opens and fully validates an existing R3 directory before adoption, shows root/project ID/revision/head hash and size, and maps managed Save to exact-current-head full-asset reopen verification without preparing or publishing a checkpoint. Dirty Legacy work is confirmed before a format transition; failed candidates preserve the current lease; terminal cleanup failures and `requiresReopen` are surfaced without leaking private diagnostics. While R3 is authoritative, legacy editors and Build/Deploy are hidden and Legacy Save As and Story actions are disabled. `gore-authoring` has closed revision-1/2/3 documents, explicit revision-2-to-3 migration, and an immutable working store with sealed heads, snapshot/entity shards, and physical Ogg/Quest-artifact CAS blobs. The visible R3 surface remains identity-only: content editing, migration orchestration, managed New/import/clone/Save As, full history/recovery, and unified all-domain transactions/content library remain missing. |
...
| Quest inspection/edit/create | **Native revision-3 prepare path proven; semantic Studio transaction/UI missing** | Mod Studio still has no semantic quest catalog, graph, or friendly revision-3 managed wizard; the separate visible managed Story flow remains schema revision 2. The revision-3 path stores compact Quest collision-artifact refs, derives a fresh capability from the exact current project and trusted game/catalog sources, and uses the committed C1 transaction plus C2 Store persistence to insert repeated Quest Draft/`ScriptModule` pairs and fully reopen an immutable candidate checkpoint. The strict native `authoring_store_prepare_revision3_quest_draft_v3` FFI route now owns that entire prepare-only sequence: it accepts closed raw transports, refreshes game/catalog authority, rechecks game and project inputs around persistence, rejects working-store/game aliasing and revisions outside the signed wire range, and never replaces the fixed head. Its response is an unpublished structural candidate with `blocked`, `runtime_unqualified`, `not_granted`, `fresh_capability_required`, and `not_supported`; a late race can leave only verified immutable CAS orphans. Structural reopen never recreates artifact/source authority. A strict Dart wrapper, managed-session semantic Quest transaction and guarded fixed-head publication, and Quest UI remain missing. A retained report of two added `UQuest` instances as `Available` proves only narrow discovery/instantiation on one generation, not transitions, dialog selection, effects, rewards, or persistence. See [quest authoring](quest-authoring.md). |
...
| Existing cooked DataAsset fixed leaves | **Native revision-3 stage proven; Studio remains read-only** | Extract, same-width compare-and-swap patch, re-inspect, and offline pack remain receipt-bound. Extract-v2/Patch-v2 wires, bounded readers, pair/USMAP/sidecar/generation validators, and opaque verified capabilities live in one shared `gore-asset` workflow used by the CLI. Native revision-3 authoring now consumes that opaque input, freshly reconstructs and union-probes the live target, binds the exact non-empty executable, rejects Store/game aliasing, and prepares a fully reopened immutable candidate containing only a closed path-free stage manifest plus patched pair, exact USMAP, and sidecars in AssetStore/CAS. Exact-head list and registry-only removal are bounded and fully reopen every historical basis; none publishes the fixed head or grants build, runtime, artifact, pack, or deployment authority. The read-only DataAsset Lab still exposes no patch, stage, save, pack, deploy, raw-offset, or semantic gameplay control. Strict FFI/Dart/session wiring and a semantic stage/diff/undo UI remain missing. See [DataAsset authoring](dataasset-authoring.md). |
