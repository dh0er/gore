...
| New NPC identity | **Offline compile/compose, managed revision-2 persistence, and native revision-3 prepare-only insertion proven; runtime missing** | A new `CharacterDefinition` with a new `UniqueName`, a linked `AIAgentConfig`, and a linked `SpawnAIAgentDefinition` compile and compose as one additive AngelScript module while leaving visual/actor defaults inherited from Asghan-derived parents. In the visible Story flow, the source-sealed picker atomically persists one schema-revision-2 NPC Draft plus deterministic `ScriptModule`. Separately, the revision-3 core now atomically inserts the same closed NPC/module pair against an exact head/project/revision/target while consuming fresh Story+NPC catalog selection and a base-game-plus-current-project collision inventory for modules, paths, symbols, and pinned-catalog runtime IDs. It regenerates the complete existing NPC/Quest/module closure, preserves valid Quests, and fails on residual/drift/collisions. Native FFI rebuilds those contexts and prepares/reopens an immutable R3 checkpoint without publishing the fixed head. Managed R3 session/Studio publication and semantic editing remain missing. Every result remains build-blocked/runtime-unqualified, with no production lowerer, spawn, or runtime workflow. Class residence, discovery, effective visuals, distinct identity, spawning, dialog/quest separation, and persistence are not proven. Runtime-ID coverage is limited to the pinned catalog projection. Spawning another body for an existing identity remains a different, weaker proof because it shares that identity's dialog/quest state. Cooked-asset creation is required for genuinely new visual/content assets and for any registry or collection change the recovered chain actually requires, but is not currently proven mandatory for a logical NPC identity. See [NPC authoring](npc-authoring.md). |
...
| Quest inspection/edit/create | **Managed revision-3 Draft transaction and first friendly wizard proven; complete semantic editor missing** | Managed R3 Home now exposes a bounded Quest Draft wizard for name, description, first objective, Quest family, and giver. It rebuilds friendly Story-catalog choices at open and immediately before publication, hides generated IDs/namespaces/paths, derives technical identities from the exact checkpoint, rejects stale-wizard publication, and refreshes the revision/content library after success. The separate visible managed Story flow remains schema revision 2. The revision-3 path stores compact Quest collision-artifact refs, derives a fresh capability from the exact current project and trusted game/catalog sources, and uses the committed C1 transaction plus C2 Store persistence to insert repeated Quest Draft/`ScriptModule` pairs and fully reopen an immutable candidate checkpoint. The strict native `authoring_store_prepare_revision3_quest_draft_v3` FFI route owns that prepare-only sequence: it accepts closed raw transports, refreshes game/catalog authority, rechecks game and project inputs around persistence, rejects working-store/game aliasing and non-signed-wire values, and never replaces the fixed head. A strict duplicate-safe Dart wrapper validates the exact basis and complete Quest/module/artifact candidate closure; the managed session then publishes by guarded fixed-head byte CAS, repair journal, and full published reopen. Its result remains `blocked`, `runtime_unqualified`, `not_granted`, `fresh_capability_required`, and `not_supported`; a late race can leave only verified immutable CAS orphans. Structural reopen never recreates artifact/source authority. The semantic Quest catalog/outline/state graph, conditions/effects, journal/reward authoring, build lowering, and runtime workflow remain missing. A retained report of two added `UQuest` instances as `Available` proves only narrow discovery/instantiation on one generation, not transitions, dialog selection, effects, rewards, or persistence. See [quest authoring](quest-authoring.md). |
...
| Existing cooked DataAsset fixed leaves | **Managed revision-3 staging transaction proven; visible Studio editor remains read-only** | Extract, same-width compare-and-swap patch, re-inspect, and offline pack remain receipt-bound. Extract-v2/Patch-v2 wires, bounded readers, pair/USMAP/sidecar/generation validators, and opaque verified capabilities live in one shared `gore-asset` workflow used by the CLI. Native revision-3 authoring consumes that opaque input, freshly reconstructs and union-probes the live target, binds the exact non-empty executable, rejects Store/game aliasing, and prepares a fully reopened immutable candidate containing only a closed path-free stage manifest plus patched pair, exact USMAP, and sidecars in AssetStore/CAS. Closed bounded FFI routes expose exact-head prepare/list/registry-only remove, reject unsafe receipt inputs and non-signed-wire nested values, and return no path, receipt, raw offset, artifact, build, runtime, pack, deployment, or native-publication authority. Strict Dart DTOs validate real native wire/manifest ordering, nested seals, complete candidate/AssetStore bindings, and closed statuses. The managed R3 session now publishes prepare/remove by serialized full reopen, crash repair, exact fixed-head byte CAS, and full published reopen; listing is an exact-head read, drift cannot clobber, and integrity uncertainty poisons. The visible read-only DataAsset Lab still exposes no project stage, semantic diff/undo, pack, deploy, or gameplay control. That UI, build lowering, and post-pack verification remain missing. See [DataAsset authoring](dataasset-authoring.md). |
...
| Build/deploy/undeploy | **Proven for represented Legacy domains; blocked for managed revision 3** | Studio drives the same bundle engine as the CLI and can restore its deployment for represented compatibility-project domains. The bounded existing-member spoken-line replacement participates in that portable Save/Reopen and Build/Deploy path. When an R3 project is current, the shell hides Legacy editors and Build/Deploy instead of reading stale provider state; cooked DataAsset authoring and managed-R3 lowering are not connected. A build dialog is not yet a project-wide dependency/risk review. |
...
deploy, raw-offset, or semantic-edit control.
...
and never replace `gore-project.json`. This is a native project transaction
boundary, not yet a Studio editing flow or a buildable package.
