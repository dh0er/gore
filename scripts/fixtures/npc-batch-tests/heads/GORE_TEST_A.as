...
class UChoiceGorePaletteFlex : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150000;
    default Caption = FText::FromString(n"H1 Gesicht: Flex".ToString());
    ...
    void Act()
    {
        AG1RGameState::SaveWorldFloatData(this.GetSelf().GetWorld(), n"gore_head_v1_mode", 1.0f);
        AGothicNPCState C = FCharacterUniqueName(n"GORE_TEST_C").GetNPCState();
        UGoreFlexHeadProbeController::GetOrCreate(C, n"GoreFlexHeadController").Initialize(C);
        this.EndConversation();
    }
}
class UChoiceGorePaletteHero : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150001;
    default Caption = FText::FromString(n"H2 Gesicht: Held".ToString());
    default PriorityRank = 29;
    UFUNCTION(BlueprintOverride)
    bool IsVisible() const { return FCharacterUniqueName(n"GORE_TEST_C").GetNPCState() != nullptr; }
    UFUNCTION(BlueprintOverride)
    void Act()
    {
        AG1RGameState::SaveWorldFloatData(this.GetSelf().GetWorld(), n"gore_head_v1_mode", 0.0f);
        AGothicNPCState C = FCharacterUniqueName(n"GORE_TEST_C").GetNPCState();
        UGoreFlexHeadProbeController::GetOrCreate(C, n"GoreFlexHeadController").Initialize(C);
        this.EndConversation();
    }
}
class UChoiceGorePaletteHairOff : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150002;
    default Caption = FText::FromString(n"H3 Haare aus".ToString());
    default PriorityRank = 28;
    UFUNCTION(BlueprintOverride)
    bool IsVisible() const { return FCharacterUniqueName(n"GORE_TEST_C").GetNPCState() != nullptr; }
    UFUNCTION(BlueprintOverride)
    void Act()
    {
        AG1RGameState::SaveWorldFloatData(this.GetSelf().GetWorld(), n"gore_palette_hair_hidden", 1.0f);
        AGothicNPCState C = FCharacterUniqueName(n"GORE_TEST_C").GetNPCState();
        UGoreFlexHeadProbeController::GetOrCreate(C, n"GoreFlexHeadController").Initialize(C);
        this.EndConversation();
    }
}
class UChoiceGorePaletteHairOn : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150003;
    default Caption = FText::FromString(n"H4 Haare an".ToString());
    default PriorityRank = 27;
    UFUNCTION(BlueprintOverride)
    bool IsVisible() const { return FCharacterUniqueName(n"GORE_TEST_C").GetNPCState() != nullptr; }
    UFUNCTION(BlueprintOverride)
...
class UChoiceGorePaletteRed : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150006;
    default Caption = FText::FromString(n"H7 Haarfarbe: Rot (Flex)".ToString());
    default PriorityRank = 24;
    UFUNCTION(BlueprintOverride)
    bool IsVisible() const { return FCharacterUniqueName(n"GORE_TEST_C").GetNPCState() != nullptr; }
    UFUNCTION(BlueprintOverride)
    void Act()
    {
        AG1RGameState::SaveWorldFloatData(this.GetSelf().GetWorld(), n"gore_palette_hair_red", 1.0f);
        AGothicNPCState C = FCharacterUniqueName(n"GORE_TEST_C").GetNPCState();
        UGoreFlexHeadProbeController::GetOrCreate(C, n"GoreFlexHeadController").Initialize(C);
        this.EndConversation();
    }
}
class UChoiceGorePaletteNatural : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150007;
    default Caption = FText::FromString(n"H8 Haarfarbe: Original (Flex)".ToString());
    default PriorityRank = 23;
    UFUNCTION(BlueprintOverride)
    bool IsVisible() const { return FCharacterUniqueName(n"GORE_TEST_C").GetNPCState() != nullptr; }
    UFUNCTION(BlueprintOverride)
...
class UChoiceGorePaletteRestore : UTopic_Hero__GORE_TEST_A
{
    default DebugId = 3409845526797150008;
    default Caption = FText::FromString(n"H9 Alles wiederherstellen".ToString());
    default PriorityRank = 10;
    UFUNCTION(BlueprintOverride)
    void Act()
    {
        AG1RGameState::SaveWorldFloatData(this.GetSelf().GetWorld(), n"gore_head_v1_mode", 0.0f);
